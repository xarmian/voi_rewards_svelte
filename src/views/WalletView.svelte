<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			const [supplyInfo, dates] = await Promise.all([getSupplyInfo(), dataTable.fetchDateRanges()]);
			...
			if (epochData) {
				...
				const rewardedBlocks =
					epochData.num_blocks + Math.min(epochData.num_blocks / 3, epochData.num_blocks_ballast);
...
<div class="max-w-4xl mx-auto">
	...
	{:else}
		<div
			class="bg-white dark:bg-gray-900 rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl p-6"
		>
			<h3
				class="text-2xl font-bold mb-4 flex items-center justify-between text-gray-800 dark:text-gray-200"
			>
				<span>Account</span>
				<div>
					<CopyComponent
					...
					{#if isModal}
						<a
							href="https://explorer.voi.network/explorer/account/{walletId}/transactions"
			...
			<h1 class="text-xl mb-4 text-center">
				...
				>
					<span
						class="font-mono text-lg overflow-hidden whitespace-nowrap text-ellipsis max-w-full"
					>
			...
			{#if nfDomain}
				<p class="text-center mb-6">
					<a
						href="https://app.nf.domains/name/{nfDomain}"
						target="_blank"
						class="text-blue-500 hover:text-blue-700 hover:underline"
					>
						{nfDomain}
...
<div class="max-w-4xl mx-auto mt-8">
	<div class="flex border-b justify-center mb-6 overflow-x-auto no-scrollbar">
		{#each tabs as tab, index}
			<button
				class="py-2 px-4 border-b-2 font-medium text-sm focus:outline-none transition-colors duration-200 whitespace-nowrap
		...
		>
			{#each tabs as tab}
				<div class="w-full flex-shrink-0">
					<svelte:component this={tab.component} {walletId} />
				</div>
			{/each}
		</div>
	</div>
