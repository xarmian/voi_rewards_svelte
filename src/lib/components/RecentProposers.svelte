<script lang="ts">
	...
	async function fetchRecentProposers() {
		try {
			...
			const newNonBallastBlocks = newProposers.filter(
				(p: Proposer) => !ballasts.includes(p.address) && !processedBlocks.has(p.block)
			).length;
	...
	<div>
		<ul class="space-y-2">
			{#each proposers as proposer, i (proposer.address + proposer.timestamp)}
				...
				>
					<a
						class="text-sm md:col-span-1 block hover:underline"
						href={`/wallet/${proposer.address}`}
					>
						{#if proposer.nfd}
							{proposer.nfd}
						{:else}
					...
					<a
						...
						href={`https://explorer.voi.network/explorer/block/${proposer.block}`}
						target="_blank">{proposer.block}</a
					>
				</li>
			{/each}
			{#if proposers.length === 0}
				{#each Array(5) as _, i}
					<li
						class="flex flex-col md:grid md:grid-cols-3 md:gap-4 text-gray-400 dark:text-gray-600 h-[46px]"
					>
