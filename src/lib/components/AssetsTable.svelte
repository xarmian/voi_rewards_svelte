<script lang="ts">
	...
	$: assets = [
		...
		...fungibleTokens.filter(isLPToken).map((token) => createUnifiedAsset(token, 'LP')),
						...
						<TableBodyCell>
							<div class="flex items-center space-x-2">
								...
								{:else if asset.type === 'LP' && asset.poolInfo?.poolId}
									<Button
										size="xs"
										href={getAddLiquidityUrl(asset.poolInfo.poolId)}
										target="_blank"
									>
										Add Liquidity
									</Button>
								...
								>
									<i
										class="fas fa-copy text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 cursor-pointer"
									></i>
								</CopyComponent>
							</div>
						</TableBodyCell>
