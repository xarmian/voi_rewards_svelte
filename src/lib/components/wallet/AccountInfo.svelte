<script lang="ts">
	...
	const blockPeriods = [
		{
			period: 'Daily',
			amount: account.expectedBlocksPerDay,
			rewards: account.estimatedRewardsPerDay
		},
		{
			period: 'Weekly',
			amount: account.expectedBlocksPerWeek,
			rewards: account.estimatedRewardsPerWeek
		},
		{
			period: 'Monthly',
			amount: account.expectedBlocksPerMonth,
			rewards: account.estimatedRewardsPerMonth
		}
	];
...
<div class="bg-white dark:bg-gray-900 rounded-lg shadow-md p-6 space-y-6 relative">
	...
	>
		<div class="flex justify-between items-start">
			<div class="space-y-2">
				<div class="flex items-center gap-2">
					{#if addressLink}
						...
						>
							...
							{:else}
								{account.address.substring(0, 12)}...{account.address.substring(
					...
					<div class="flex items-center gap-1">
						...
						<button
							...
							title={$favorites.includes(account.address)
				...
				{#if showNicknameInput}
					<div
						...
						role="presentation"
					>
						<NicknameManager address={account.address} bind:showInput={showNicknameInput} />
					</div>
				{/if}

				<div class="flex items-center gap-2">
					<span
						class={`px-2 py-0.5 text-sm rounded-full ${
			...
			<div
				class="md:hidden transform transition-transform duration-200 absolute top-5 right-5"
		...
		<div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-lg w-full shadow-xl">
			...
			<div class="space-y-4">
				<p class="text-gray-600 dark:text-gray-300">
					This account is currently offline and not participating in consensus. The values shown are
					estimates of what could be earned if the account was participating in consensus.
				</p>
				<p class="text-gray-600 dark:text-gray-300">
