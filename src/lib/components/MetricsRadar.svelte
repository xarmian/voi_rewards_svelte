<script lang="ts">
	...
	async function processTokenData() {
		...
		tokenRadarData = processedData
			.filter(
				(result): result is PromiseFulfilledResult<TokenRadarData> => result.status === 'fulfilled'
			)
...
<Card class="p-6">
	...
	<div class="flex items-center justify-between mb-6">
		<div>
			<h3 class="text-xl font-bold text-gray-900 dark:text-white">Token Metrics Radar</h3>
	...
	<div class="flex gap-6">
							...
							{#if tokenData.visible}
								...
								{#each radarMetrics as metric, metricIndex}
									{@const coords = getPointCoordinates(
										metricIndex,
										tokenData.normalizedValues[metric.key] || 0
									)}
									<circle
										cx={coords.x}
										cy={coords.y}
									...
									>
										<title>
											{tokenData.token.symbol} - {metric.label}: {metric.format(
												tokenData.values[metric.key] || 0
											)}
										</title>
									</circle>
		...
		<div class="w-64 space-y-4">
						...
						{#each tokenRadarData as tokenData (tokenData.token.id)}
							...
							>
								<div class="flex items-center gap-2">
									<input type="checkbox" bind:checked={tokenData.visible} class="rounded" />
									<div
			...
			<!-- Metrics legend -->
			<div>
				<h4 class="font-semibold text-gray-900 dark:text-white mb-3">Metrics</h4>
				<div class="space-y-2">
					{#each radarMetrics as metric}
						<div
							class="p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer {hoveredMetric ===
							metric.key
			...
			{#if hoveredToken}
				<div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3" transition:fade>
					<h5 class="font-semibold text-gray-900 dark:text-white mb-2 flex items-center gap-2">
						<div class="w-3 h-3 rounded-full" style="background-color: {hoveredToken.color}"></div>
						{hoveredToken.token.symbol}
					</h5>
