...
<div class="flex flex-col space-y-6 mt-6">
	<div>
		...
		{#if quest.frequency && quest.frequency !== 'Once'}
			<div class="text-sm">
				(Resets at midnight UTC{quest.frequency === 'Weekly' ? ' on Monday mornings' : ''})
			</div>
		{/if}
	</div>
	...
	<div class="pt-2">
		{#if !project.realtime || quest.earned == -1 || !quest.name}
			<div class="text-sm text-red-500 flex items-center space-x-2">
				...
				<InfoButton noAbsolute={true} buttonColor="text-[#0A77FF]">
					<p class="text-xs">
						Completion status is not currently available for this quest. Quests or actions listed
						should still be tracked if done properly, and may be visible on other platforms such as
						Galxe, on the Project's own page, or not available yet. A link to the project's Galaxe
						page is available above this quest table.
					</p>
				</InfoButton>
			</div>
		{:else if !wallet}
		...
		{:else if quest.status === 'done'}
			<div class="flex items-start space-x-2 text-gray-500">
				...
				<span class="flex space-x-2">
					...
					<InfoButton noAbsolute={true} buttonColor="text-[#0A77FF]">
						<p class="text-sm">
							This quest has expired and is no longer available to complete. Please check the
							project's page for more information. Expired quests may be re-instated in the future.
						</p>
					</InfoButton>
				</span>
			</div>
		...
		{#if quest.earned && quest.earned > 0}
			<div class="text-sm text-green-800">
				+{quest.earned} point{(quest.earned ?? 0) > 1 ? 's' : ''}
			</div>
		{/if}
	</div>
</div>
