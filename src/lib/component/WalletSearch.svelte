<script lang="ts">
	import {
		getEnvoiAddresses,
		searchEnvoi,
		type EnvoiSearchResult,
		getEnvoiNames
	} from '$lib/utils/envoi';
	...
	function populateConnectedWallets() {
		...
		addressList = wallets
			.map((address) => ({
				address,
				name:
					$connectedWallets.find((w) => w.address === address)?.envoiName ||
...
<div class="relative" bind:this={componentElement}>
	<div class="dark:bg-gray-800 bg-white flex items-center rounded-lg relative">
		<input
			...
			class="dark:bg-gray-700 bg-gray-100 flex-grow p-2 pr-8 text-black dark:text-white {hideSubmitButton
				? 'rounded-lg'
				: 'rounded-l-lg'}"
			...
			aria-activedescendant={selectedAddressIndex >= 0
				? `option-${selectedAddressIndex}`
				: undefined}
		/>
		{#if searchText}
			<button
				class="absolute inset-y-0 {hideSubmitButton
					? 'right-2'
					: 'right-16'} pr-0 flex items-center cursor-pointer"
		...
		{#if !hideSubmitButton}
			<button
				...
				class="bg-[#6F2AE2] border border-bg-[#6F2AE2] text-white p-2 rounded-r-lg"
			>
	...
	{#if addressList.length > 0}
		<div
			class="fixed z-50"
			style="top: {componentElement?.getBoundingClientRect().bottom +
				8}px; left: {componentElement?.getBoundingClientRect()
				.left}px; width: {componentElement?.getBoundingClientRect().width}px;"
		>
			<div
				class="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-h-64 border border-gray-200 dark:border-gray-700 w-full"
			>
				<ul
					id="search-listbox"
					role="listbox"
				...
				>
					{#each addressList as result, index}
						<li>
							<button
								class="p-2 border border-solid text-left w-full hover:bg-gray-200 dark:hover:bg-gray-700 hover:border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white {index ===
								selectedAddressIndex
									? 'bg-purple-100 dark:bg-purple-900 ring-2 ring-[#6F2AE2] ring-opacity-50'
									: ''}"
							...
							>
								<div class="flex items-center gap-3">
									...
									{:else}
										...
										>
											<span
												class="text-gray-600 dark:text-gray-300 text-sm w-8 h-8 flex items-center justify-center"
											>
