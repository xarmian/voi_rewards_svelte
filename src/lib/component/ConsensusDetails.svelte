...
<div class="flex items-center justify-end gap-2 mb-2">
	<InfoButton noAbsolute customTrigger buttonColor="dark:text-gray-400 text-gray-600">
		<div slot="trigger" class="flex items-center gap-2 cursor-help">
			<span
				class="w-2 h-2 rounded-full {accountStatus === 'Online' ? 'bg-green-500' : 'bg-gray-500'}"
			></span>
			<p class="text-sm font-medium text-gray-700 dark:text-gray-300">{accountStatus}</p>
		</div>
		{#if accountStatus === 'Online'}
			This account is participating in consensus and can earn block rewards. Click the voting status
			button for more details.
		{:else}
			This account is not participating in consensus. To earn block rewards, you need to register a
			participation key.
		{/if}
	</InfoButton>
	{#if accountStatus === 'Online' && consensusDetails}
		<button
			...
			class="text-sm px-2 py-1 rounded-md {consensusHealth === 'good'
...
{#if accountStatus === 'Online' && isConsensusDetailsExpanded}
	...
	>
				...
				<div>
					...
					<div class="space-y-1">
						<div class="flex justify-between">
							...
							<span class="text-gray-900 dark:text-white"
								>{consensusDetails.vote_count.toLocaleString()}</span
							>
						</div>
						<div class="flex justify-between">
							<span class="text-gray-600 dark:text-gray-400">Total Blocks</span>
							<span class="text-gray-900 dark:text-white"
								>{consensusDetails.total_blocks.toLocaleString()}</span
							>
						</div>
							...
							<div class="flex justify-between">
								...
								<span class="text-gray-900 dark:text-white">
									...
									{:else if timeSinceLastVote < 86400}
										{Math.floor(timeSinceLastVote / 3600)}h {Math.floor(
											(timeSinceLastVote % 3600) / 60
										)}m ago
									{:else}
										{Math.floor(timeSinceLastVote / 86400)}d {Math.floor(
											(timeSinceLastVote % 86400) / 3600
										)}h ago
									{/if}
								</span>
							</div>
		...
		{#if consensusHealth === 'warning' || consensusHealth === 'error' || consensusHealth === 'unknown'}
			<div
				class="mt-3 p-2 rounded {consensusHealth === 'warning'
			...
			>
				<div class="flex items-center">
					<i
						class="fas {consensusHealth === 'unknown'
							? 'fa-info-circle'
					...
					></i>
					<p
						class="text-xs {consensusHealth === 'warning'
					...
					>
						{#if consensusHealth === 'unknown'}
							Due to your current stake being less than 1,000 VOI, we cannot accurately estimate
							voting health.
						{:else if consensusHealth === 'warning'}
							Your node's voting frequency is slower than expected for your stake size. This might
							affect your rewards.
						{:else}
							Your node has not voted in a significant period. Please check your node's status
							immediately.
						{/if}
					</p>
				</div>
