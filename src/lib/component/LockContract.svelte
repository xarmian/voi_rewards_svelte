...
>
	<!-- Contract Header -->
	<div
		class="px-4 lg:px-6 py-3 lg:py-4 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 border-b border-gray-200 dark:border-gray-700"
	>
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-2 lg:gap-4 w-full justify-between">
				...
				<div class="flex gap-2">
					<span
						class={`inline-flex items-center px-2 lg:px-3 py-0.5 lg:py-1 rounded-full text-xs lg:text-sm font-medium ${
					...
					>
						<i
							class={`fas ${contract.isParticipating ? 'fa-check-circle' : 'fa-times-circle'} mr-1 lg:mr-1.5`}
						></i>
		...
		<div class="lg:grid lg:grid-cols-3 lg:gap-6 space-y-4 lg:space-y-0">
					...
					<div class="space-y-2">
						<div class="flex justify-between items-center">
							...
							<span class="font-medium text-gray-900 dark:text-white"
								>{formatNumber(contract.availableForWithdrawal)} VOI</span
							>
						</div>
						<div class="flex justify-between items-center">
							<span class="text-gray-600 dark:text-gray-300">Total</span>
							<span class="font-medium text-gray-900 dark:text-white"
								>{formatNumber(contract.balance)} VOI</span
							>
						</div>
						<div class="flex justify-between items-center">
							<span class="text-gray-600 dark:text-gray-300">Locked</span>
							<span class="font-medium text-gray-900 dark:text-white"
								>{formatNumber(contract.lockedBalance)} VOI</span
							>
						</div>
					</div>
				</div>
			...
			>
				...
				<div class="space-y-2">
					<div class="flex justify-between items-center">
						...
						<span class="font-medium text-gray-900 dark:text-white"
							>{formatDuration(contract.lockupPeriod)} Lock / {formatDuration(
					...
					{#if contract.nextVestingDate}
						<div class="flex justify-between items-center">
							...
							<span class="font-medium text-gray-900 dark:text-white"
								>{formatDate(contract.nextVestingDate)}</span
							>
						</div>
						<div class="flex justify-between items-center">
							<span class="text-gray-600 dark:text-gray-300">Next Amount</span>
							<span class="font-medium text-gray-900 dark:text-white"
								>{formatNumber(contract.nextVestingAmount)} VOI</span
							>
						</div>
					{/if}
					<div class="flex justify-between items-center">
						...
						<span class="font-medium text-gray-900 dark:text-white"
							>{formatNumber(contract.remainingVestingAmount)} VOI</span
						>
					</div>
				</div>
			</div>
			...
			>
				...
				<div class="space-y-2">
						...
						{#if contract.accountInfo?.participation}
							<div class="flex justify-between items-center">
								...
								<span class="font-medium text-gray-900 dark:text-white">
									{formatDate(
										new Date(
											(contract.accountInfo?.participation['vote-last-valid'] -
												contract.accountInfo?.round) *
												2.8 *
												1000 +
												Date.now()
										)
					...
					{:else}
						<div
							class="text-sm text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 p-3 rounded-lg"
						>
							<i class="fas fa-info-circle mr-2"></i>
							This contract is not currently participating in consensus. Register a participation key
