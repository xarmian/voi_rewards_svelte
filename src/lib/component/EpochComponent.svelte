<script lang="ts">
	...
	const loadEpochData = async () => {
		try {
			...
			return (await Promise.all(epochPromises)).filter(
	...
	$: {
		if (walletAddress) {
			isLoading = true;
			Promise.all([loadTransactions(), loadEpochData()])
	...
	{:else}
		<Tabs style="underline">
			<TabItem open>
				...
				<EpochTable {paginatedData} bind:currentPage {totalPages} {dataLastUpdated} />
			</TabItem>
			<TabItem>
				<div slot="title" class="flex items-center gap-2">
