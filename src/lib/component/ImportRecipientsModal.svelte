<script lang="ts">
	...
	function autoDetectColumns() {
		...
		for (let i = 0; i < columnCount; i++) {
			...
			if (
				firstRow &&
				(firstRow.includes('note') || firstRow.includes('message') || firstRow.includes('memo'))
			) {
				noteColumnIndex = i;
	...
	async function resolveVoiNames() {
		...
		try {
			...
			const resultMap = new Map(data.results.map((result) => [result.name.toLowerCase(), result]));
	...
	async function fetchCollectionInfo(id: string, round?: string) {
		...
		try {
			...
			const validTokens = data.tokens.filter(
				(token: NFTToken) => !token.isBurned && token.owner !== zeroAddress
			);
	...
	async function fetchArc200TokenInfo(id: string) {
		...
		try {
			...
			arc200TokenHolders = allBalances.filter(
				(balance: any) =>
	...
	async function handleArc200TokenImport() {
		...
		if (
			arc200DistributionType === 'proportional' &&
			(!amountValue || isNaN(Number(amountValue)) || Number(amountValue) <= 0)
		) {
	...
	<div class="w-full flex flex-col max-h-[calc(100vh-4rem)]">
		<div class="flex-none">
			<div class="flex items-center justify-between mb-6">
				<h3 class="text-xl font-bold text-gray-900 dark:text-white">Import Recipients</h3>
				{#if importStep === 'preview'}
			...
			{#if error}
				<div
					class="p-4 mb-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 rounded-lg text-sm"
				>
			...
			{#if importStep === 'preview'}
				<div class="mb-6 grid grid-cols-2 gap-4">
					<div>
						...
						<Select class="w-full" bind:value={addressColumnIndex}>
					...
					<div>
						...
						<Select class="w-full" bind:value={amountColumnIndex}>
		...
		<div class="flex-1 min-h-0 overflow-auto">
			{#if importStep === 'initial'}
				<div
					class="h-96 border-2 border-dashed rounded-lg p-8 text-center flex items-center justify-center {isDragging
					...
					{:else}
						<div class="w-full max-w-lg mx-auto space-y-6">
							...
							<div class="flex flex-col gap-3">
								...
								<Button color="light" class="w-full" on:click={handlePasteFromClipboard}>
			...
			{:else if importStep === 'nft-collection'}
				<div
					class="flex items-center justify-center place-self-center border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg px-8 py-8 sm:px-28 text-center"
				>
					<div class="w-full max-w-lg mx-auto space-y-6">
						<div class="text-gray-500 dark:text-gray-400">
							...
							<p class="text-sm">
								Enter a collection ID or paste an <a
									href="https://nftnavigator.xyz/"
									target="_blank"
									class="text-blue-500">NFT Navigator</a
								> collection URL
							</p>
						</div>
						...
						<div class="space-y-6">
							...
							{#if collectionError && collectionId.trim() && !isLoadingCollectionInfo}
								<div
									class="p-3 text-sm bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 rounded-lg"
								>
									<div class="flex items-center gap-2">
										<i class="fas fa-exclamation-circle"></i>
							...
							{#if collectionName || collectionImageUrl}
								<div
									class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-900/40 hover:outline hover:outline-gray-300 dark:hover:outline-gray-700"
								>
									<a
										href={`https://nftnavigator.xyz/collection/${collectionId.includes('collection/') ? collectionId.split('collection/')[1].split(/[^0-9]/)[0] : collectionId}`}
										target="_blank"
											...
											{#if collectionName}
												<div class="flex-1">
													...
													<p class="text-sm text-gray-500 dark:text-gray-400">
														Collection #{collectionId.includes('collection/')
															? collectionId.split('collection/')[1].split(/[^0-9]/)[0]
															: collectionId}
													</p>
							...
							<div class="space-y-2">
								...
								{#if amountType !== 'none'}
									<div class="mt-2">
										<Input
											type="number"
											placeholder={amountType === 'per-holder'
												? 'Amount per holder'
												: amountType === 'per-nft'
													? 'Amount per NFT'
							...
							<div class="flex justify-end gap-3">
								...
								>
									...
									{#if !isCollectionValid && !isLoadingNFTHolders && collectionId.trim()}
										<div
											class="hidden group-hover:block absolute -top-10 left-1/2 transform -translate-x-1/2 w-48 p-2 bg-gray-900 text-white text-xs rounded-lg"
										>
								...
								>
									<div class="flex items-start gap-4">
										...
										<div class="flex-1">
											<h4 class="font-medium text-gray-900 dark:text-white">
												{arc200TokenName}
												{arc200TokenSymbol ? `(${arc200TokenSymbol})` : ''}
											</h4>
											<p class="text-sm text-gray-500 dark:text-gray-400">
								...
								{#if arc200DistributionType !== 'equal'}
									<div class="mt-2">
										...
										<Input
											...
											type="number"
											placeholder={arc200DistributionType === 'per-holder'
												? 'Amount per holder'
												: 'Total amount to distribute'}
											bind:value={amountValue}
										/>
										...
										<td class="p-4">
											<div class="flex items-center gap-2">
												...
												{#if !isValidAddressContent(row.cells[addressColumnIndex])}
													<i
														class="fas fa-ban text-red-500 dark:text-red-400"
														title="This row will be skipped during import due to invalid address"
													></i>
												{/if}
											</div>
										</td>
												...
												{#if j === addressColumnIndex && row.cells[j]}
													<div class="flex flex-col gap-1">
														<div class="flex items-center gap-2">
															...
															{#if !row.isValid}
																<i
																	class="fas fa-exclamation-circle text-red-600 dark:text-red-400"
														...
														{#if row.cells[j].endsWith('.voi')}
															{#if row.resolvedAddress}
																<div class="text-xs text-gray-500 dark:text-gray-400 font-normal">
																	{row.resolvedAddress.slice(0, 8)}...{row.resolvedAddress.slice(
																		-8
																	)}
																</div>
															{:else}
