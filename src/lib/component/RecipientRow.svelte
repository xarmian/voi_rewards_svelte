...
<div class="p-3 relative">
	...
	<div class="flex-1 pr-8">
		<div
			class="grid grid-cols-[minmax(200px,1fr),minmax(80px,120px)] sm:grid-cols-[minmax(280px,420px),120px,auto] gap-2 sm:gap-4"
		>
			<!-- Recipient Column -->
			<div class="space-y-2">
				<WalletSearch
								...
								<div class="flex flex-wrap items-center gap-x-4 gap-y-2">
									...
									{#if !isNativeToken(selectedToken)}
										<div class="flex items-center gap-2">
											...
											<span class="text-sm text-gray-700 dark:text-gray-300">
												{(
													(recipient.info.assetBalance ?? 0) / Math.pow(10, selectedToken.decimals)
												).toLocaleString()}
											</span>
										</div>
									{/if}
			...
			<div>
				<div class="relative flex items-center gap-2">
					...
					<div class="relative flex-shrink-0">
						<button
							type="button"
							class="p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-150 {recipient.note
						...
						>
							...
							{#if recipient.note}
								<span
									class="absolute top-0 right-0 block w-2 h-2 bg-blue-500 dark:bg-blue-400 rounded-full"
								></span>
							{/if}
						</button>
						...
						{#if isNoteVisible}
							<div
								...
								style="left: {calculatePopupPosition().left}px; top: {calculatePopupPosition()
									.top}px;"
							...
							>
								...
								<div
									class="absolute -top-2 right-6 w-4 h-4 bg-white dark:bg-gray-800 border-t border-l border-gray-200 dark:border-gray-700 transform rotate-45"
								></div>
								...
								<div class="p-3 relative">
									<div class="flex justify-between items-center mb-2">
										<label
											...
											>Add a note for this recipient</label
										>
										{#if recipient.note}
											<button
												type="button"
