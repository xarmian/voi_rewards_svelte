<script lang="ts">
	...
	$: canSignTransactions =
		$selectedWallet?.address === walletId &&
		$selectedWallet?.app !== '' &&
	...
	async function loadApprovals() {
		...
		try {
			...
			outgoingApprovals = allApprovals.filter(
				(approval) => approval.contractId.toString() === token.id
			);
		} catch (error) {
	...
	<div class="fixed inset-0 bg-black bg-opacity-50 z-40 flex items-center justify-center p-4">
		...
		>
			<div class="p-5">
				...
				{#if !isArc200}
					<div
						class="bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 p-4 rounded-lg mb-4"
					>
						...
						<div class="space-y-4">
							...
							<div>
								...
								<div class="flex items-center">
									...
									/>
									<span
										class="px-3 py-2 border border-l-0 border-gray-300 dark:border-gray-600 rounded-r-md bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-300"
									>
										{tokenSymbol}
									</span>
								</div>
								<p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
									Your balance: {formatTokenAmount(token.balance.toString(), tokenDecimals)}
									{tokenSymbol}
								</p>
							</div>
							...
							>
								{#if isApproving}
									<div
										class="w-4 h-4 border-t-2 border-r-2 border-white rounded-full animate-spin mr-2"
									></div>
									Creating Approval...
								{:else}
									Create Approval
					...
					<div>
						<div class="flex justify-between items-center mb-3">
							...
							>
								{#if isRefreshing}
									<div
										class="w-3 h-3 border-t-2 border-r-2 border-blue-500 rounded-full animate-spin"
									></div>
								{:else}
						...
						{#if isLoading}
							<div class="flex justify-center items-center py-8">
								<div
									class="w-6 h-6 border-t-2 border-r-2 border-blue-500 rounded-full animate-spin"
								></div>
							</div>
										...
										<div class="grid grid-cols-2 gap-2 text-sm">
											...
											<div>
												...
												<span class="text-gray-700 dark:text-gray-200 ml-1">
													{formatTokenAmount(approval.amount, tokenDecimals)}
													{tokenSymbol}
												</span>
											</div>
											<div>
				...
				{#if approvalError}
					<div
						class="mt-4 p-3 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 rounded-lg"
					>
				...
				{#if approvalSuccess && successTxId}
					<div
						class="mt-4 p-3 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 rounded-lg"
					>
						<p class="text-sm">
