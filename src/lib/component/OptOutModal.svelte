<script lang="ts">
	...
	async function handleOptOut() {
		...
		if ($selectedWallet?.app === '') {
			optOutError =
				'Wallet not found. Please re-select your account and/or ensure you are not using a Watch account.';
			return;
		}
		...
		try {
			...
			optOutError =
				error instanceof Error
					? error.message
					: 'Failed to opt out of the asset. Please try again.';
		} finally {
			isOptingOut = false;
		}
	}
	...
	<div class="text-center">
		...
		{#if !optOutSuccess}
			<p class="mb-6 text-gray-700 dark:text-gray-300">
				You are about to opt out of {token.symbol} ({token.id}). This will remove the asset from
				your account and decrease your minimum balance requirement by 0.1 VOI. Any remaining balance
				of this asset in your account will be burned. You may opt back into the asset in the future.
			</p>
			...
			<div class="flex justify-center gap-4">
				<Button color="red" on:click={handleOptOut} disabled={isOptingOut}>
					{#if isOptingOut}
						<svg
				...
				<Button color="alternative" on:click={resetOptOutState} disabled={isOptingOut}
					>Cancel</Button
				>
			</div>
		{:else}
