...
<div class="flex items-center justify-between">
	...
	{:else}
		<p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
			To manage participation keys, connect {parentWalletId && parentWalletId !== walletId
				? 'to the parent account'
				: 'your wallet'} using the button in the top right.
		</p>
	{/if}
</div>
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
	<div
		class="bg-white dark:bg-gray-900 rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl"
	>
		<div class="p-6">
			<h3 class="text-xl font-bold mb-4 text-gray-800 dark:text-gray-200">
				<span class="flex items-center justify-between">
					...
					<InfoButton noAbsolute>
						<p>
							This is the calculated average block proposals and rewards for the selected account
							based on account balance and the total online stake.
						</p>
					</InfoButton>
				</span>
			</h3>
				...
				<div class="space-y-3">
					...
					{:else}
						...
						<p
							class="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700"
						>
							<span class="font-medium text-gray-600 dark:text-gray-400 self-start"
								>Avg Blocks per day:</span
							>
						...
						>
							<span class="font-medium text-gray-600 dark:text-gray-400 self-start"
								>Avg Blocks per week:</span
							>
							<span class="text-gray-800 dark:text-gray-200 flex flex-col items-end">
								{expectedBlocksPerWeek.toLocaleString(undefined, { maximumFractionDigits: 1 })}
								<span class="text-gray-600 dark:text-gray-400 ml-2">
									~{estimatedRewardsPerWeek.toLocaleString(undefined, { maximumFractionDigits: 2 })}
									VOI
								</span>
							</span>
						</p>
						<p class="flex justify-between items-center py-2">
							<span class="font-medium text-gray-600 dark:text-gray-400 self-start"
								>Avg Blocks per month:</span
							>
							<span class="text-gray-800 dark:text-gray-200 flex flex-col items-end">
								{expectedBlocksPerMonth.toLocaleString(undefined, { maximumFractionDigits: 1 })}
								<span class="text-gray-600 dark:text-gray-400 ml-2">
									~{estimatedRewardsPerMonth.toLocaleString(undefined, {
						...
						<p
							class="justify-between items-center py-2 border-t border-gray-200 dark:border-gray-700"
						>
							<span class="font-medium text-gray-600 dark:text-gray-400">Approx APR:</span>
							<span class="text-gray-800 dark:text-gray-200"
								>{((estimatedRewardsPerWeek / balance) * 1e6 * 52 * 100).toFixed(2)}%</span
							>
						</p>
					{/if}
				</div>
				...
				<div class="space-y-3">
					...
					{:else}
						...
						>
							...
							<span class="text-gray-800 dark:text-gray-200"
								>{formatTime(
									(accountInfo['participation']['vote-last-valid'] - accountInfo['round']) * 2.8
								)}</span
							>
						</p>
						<p
						...
						<p
							class="flex justify-between items-center py-2 border-b-2 border-gray-400 dark:border-gray-800"
						>
							<span class="font-medium text-gray-600 dark:text-gray-400">Expiry Block:</span>
							<span class="text-gray-800 dark:text-gray-200"
								>{accountInfo['participation']['vote-last-valid'].toLocaleString()}</span
							>
						</p>
						<p
							class="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700"
						>
							...
							<span class="text-gray-800 dark:text-gray-200"
								>{apiData && apiData.vote_count ? apiData.vote_count : 'Loading...'}</span
							>
						</p>
						<p class="flex justify-between items-center py-2">
			...
			<div class="p-6">
				<div class="space-y-3">
					<p class="text-gray-600 dark:text-gray-400">
						NOTE: This account is not currently participating in consensus. Register a participation
						key to begin participating.
					</p>
				</div>
			</div>
		</div>
