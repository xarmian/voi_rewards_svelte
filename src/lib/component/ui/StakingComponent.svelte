	...
	async function loadStakingContract() {
		...
		try {
			...
			if (contractData) {
				...
				const fundingTime = contractData.global_funding
					? Number(contractData.global_funding)
					: null;
				...
				if (!fundingTime) {
					...
					const vestingDelay =
						Math.max(1, Number(contractData.global_vesting_delay)) *
						Number(contractData.global_period_seconds);
					...
					if (elapsedTime < lockupPeriod) {
						...
						const vestingStartTime =
							lockupPeriod === 0 ? fundingTime : fundingTime + fullVestingTime;
						const vestingElapsedTime = now - vestingStartTime;
						const periodsPassed = Math.floor(
							vestingElapsedTime / Number(contractData.global_distribution_seconds)
						);
						...
						if (periodsPassed >= Number(contractData.global_distribution_count)) {
							...
							// In vesting period
							const nextVestingTime =
								vestingStartTime +
								(periodsPassed + 1) * Number(contractData.global_distribution_seconds);
...
{#if minimal}
	...
	{:else if contractData}
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			<div class="space-y-2.5">
				...
				<div class="flex flex-wrap justify-between items-center gap-x-4">
					...
					<span class="text-sm font-medium text-gray-900 dark:text-white text-right flex-1"
						>{formatNumber(lockedBalance)} VOI</span
					>
				</div>
				<div class="flex flex-wrap justify-between items-center gap-x-4">
					...
					>
					<span class="text-sm font-medium text-gray-900 dark:text-white text-right flex-1"
						>{formatNumber(availableForWithdrawal)} VOI</span
					>
				</div>
			</div>
			...
			{#if nextVestingDate && !isFullyVested}
				<div
					class="space-y-2.5 pt-2 md:pt-0 border-t md:border-t-0 border-gray-100 dark:border-gray-700"
				>
					{#if lockedBalance === Number(contractData?.global_total)}
						<!-- In Lockup Period -->
						<div class="flex flex-wrap justify-between items-center gap-x-4">
						...
						<div class="flex flex-wrap justify-between items-center gap-x-4">
							...
							<span class="text-sm font-medium text-gray-900 dark:text-white text-right flex-1">
								{formatNumber(
									Number(contractData.global_total) / Number(contractData.global_distribution_count)
								)} VOI/mo
							</span>
						</div>
						<div class="flex flex-wrap justify-between items-center gap-x-4">
	...
	{:else}
		<div class="text-gray-500 dark:text-gray-400 text-sm">No staking contract found</div>
	{/if}
{:else}
							...
							{:else if nextVestingDate}
								...
								{:else if countdown}
									<div class="px-4 py-2 mt-2">
										<p
											class="font-mono text-3xl font-bold text-blue-400 tabular-nums tracking-wider"
										>
											{countdown}
										</p>
									</div>
						...
						{#if nextVestingAmount > 0 && lockedBalance !== Number(contractData?.global_total)}
							<div class="flex flex-col items-start md:items-end gap-1">
								...
								<span class="text-2xl font-bold text-gray-900 dark:text-white"
									>{formatNumber(nextVestingAmount)} VOI</span
								>
							</div>
						{/if}
					</div>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 space-x-6">
					...
					<div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
						...
						<p class="text-2xl font-bold text-gray-900 dark:text-white">
							{formatNumber(currentBalance)} VOI
						</p>
					</div>
					...
					<div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
						...
						<p class="text-2xl font-bold text-gray-900 dark:text-white">
							{formatNumber(lockedBalance)} VOI
						</p>
					</div>
					...
					<div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
						...
						<p class="text-2xl font-bold text-gray-900 dark:text-white">
							{formatNumber(availableForWithdrawal)} VOI
						</p>
					</div>
					...
					<div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
						...
						<p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
							{Number(contractData.global_distribution_count)} distribution{Number(
								contractData.global_distribution_count
							) === 1
								? ''
								: 's'}
						</p>
