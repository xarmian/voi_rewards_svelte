	...
	<div class="w-full overflow-hidden rounded-lg shadow-xs">
		<div class="w-full overflow-x-auto">
			<table class="w-full whitespace-no-wrap">
				<thead>
					<tr
						class="text-xs font-semibold tracking-wide text-left text-gray-500 dark:text-gray-100 uppercase border-b bg-gray-50 dark:bg-gray-700"
					>
					...
					{#each transfers as transfer}
						<tr class="text-gray-700 dark:text-gray-100">
							...
							{#if !isMobile}
								<td class="px-4 py-3"
									><A
										href="https://explorer.voi.network/explorer/transaction/{transfer.transactionId}"
										target="_blank">{transfer.transactionId.substring(0, 15)}...</A
									></td
								>
								<td class="px-4 py-3"
									><A
										href="https://explorer.voi.network/explorer/block/{transfer.round.toString()}"
										target="_blank">{transfer.round}</A
									></td
								>
							{/if}
							<td class="px-4 py-3 flex">
								{#if transfer.from == zeroAddress}
									<span
										class="text-center w-full text-sm bg-yellow-200 rounded-lg text-yellow-400 font-bold p-1 pl-2 pr-2"
										>Minted</span
									>
								{:else}
									<A href="/arc72/portfolio/{transfer.from}"
										>{nfdMap[transfer.from] ? nfdMap[transfer.from] : formatAddr(transfer.from)}</A
									>
								{/if}
							</td>
							<td class="px-4 py-3"
								><A href="/arc72/portfolio/{transfer.to}"
									>{nfdMap[transfer.to] ? nfdMap[transfer.to] : formatAddr(transfer.to)}</A
								></td
							>
						</tr>
					{/each}
