<script lang="ts">
	...
	onMount(async () => {
		...
		formattedOwner = token.ownerNFD
			? (token.ownerNFD as string)
			: token.owner.length > 16
	...
	tokenProps = Object.keys(token.metadata.properties).map((key) => {
		const colors = propColor(
			token.metadata.properties[key as keyof typeof token.metadata.properties]
		);
		return {
			trait_type: key,
			value: token.metadata.properties[key as keyof typeof token.metadata.properties],
			fgcolor: colors[1],
			bgcolor: colors[0]
		};
	});
	...
	function propColor(value: string) {
		...
		const index = bgcolors.includes(value)
			? bgcolors.indexOf(value)
			: Math.floor(Math.random() * bgcolors.length);

		return [softBgColors[index], softFgColors[index]];
	}

	let formattedApproved = token.approved
		? token.approved.length > 8
...
<div class="flex" class:flex-col={isMobile}>
	...
	<div class="text-left">
		...
		<div class="text-2xl font-bold mb-2 text-purple-900 dark:text-purple-100">
			<A href="/arc72/collection/{token.contractId}/token/{token.tokenId}">{token.metadata.name}</A>
		</div>
		<div class="mb-2">
			<div>
			...
			{#if token.approved && token.approved != 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAY5HFKQ'}
				<div>
					Approved Spender: <A href="/arc72/portfolio/{token.approved}">{formattedApproved}</A>
				</div>
			{/if}
			<div>
				Mint Round: <A
					href="https://explorer.voi.network/explorer/block/{token.mintRound}/transactions"
					target="_blank">{token.mintRound}</A
				>
			</div>
		...
		<div class="flex flex-wrap">
			{#each tokenProps as prop}
				<div
					style="color: {prop.fgcolor}; background-color: {prop.bgcolor}"
					class="p-2 m-1 text-md rounded-xl"
				>
