<script lang="ts">
		...
		if (isVoiToAlgorand) {
			...
			try {
				...
				try {
					...
					bridgeStatus = 'error';
					bridgeError =
						error instanceof Error ? error.message : 'Transaction signing failed or was rejected';
					return;
				}
				...
				try {
					...
					bridgeStatus = 'error';
					bridgeError =
						error instanceof Error ? error.message : 'Failed to submit transaction to network';
					return;
				}
	...
	async function startBridgeMonitoring(sourceTxId: string) {
		...
		const checkDestinationTx = async () => {
			try {
				...
				const monitorAddress = isVoiToAlgorand ? algorandDestinationAddress : voiDestinationAddress;
				...
				for (const tx of transactions) {
					try {
						...
						if (
							noteData.sourceTxId === sourceTxId &&
							noteData.sourceNetwork === expectedSourceNetwork
						) {
	...
	function handleClickOutside(event: MouseEvent) {
		if (
			...
			searchInputRef &&
			!searchInputRef.contains(event.target as Node)
		) {
			showSearchResults = false;
	...
	function handleKeydown(event: KeyboardEvent) {
		...
		switch (event.key) {
			...
			case 'ArrowUp':
				...
				if (showConnectedWallets) {
					selectedSearchIndex =
						selectedSearchIndex <= 0 ? uniqueConnectedWallets.length - 1 : selectedSearchIndex - 1;
				} else {
					selectedSearchIndex =
	...
	async function checkAlgorandBalance(address: string) {
		...
		try {
			const response = await fetch(
				`https://mainnet-api.4160.nodely.dev/v2/accounts/${address}/assets/${selectedToken.algorandAssetId}`
			);
	...
	function handleSourceKeydown(event: KeyboardEvent) {
		...
		switch (event.key) {
			...
			case 'ArrowUp':
				event.preventDefault();
				selectedSourceSearchIndex =
					selectedSourceSearchIndex <= 0
						? sourceSearchResults.length - 1
	...
	$: isValidAmount =
		...
		Number(amount) > 0 &&
		Number(amount) <=
			(isVoiToAlgorand ? voiBalance || 0 : algorandSourceBalance || Number.MAX_VALUE);
	...
	$: isFormValid =
		...
		(!isVoiToAlgorand ||
			...
			(algorandSourceBalance !== null && algorandSourceBalance >= Number(amount)));
	...
	$: submitButtonTooltip = !isValidDestination
		...
		: !isValidAmount
			...
			: isVoiToAlgorand &&
				  isValidSourceAddress &&
				  algorandSourceBalance !== null &&
				  algorandSourceBalance < Number(amount)
					...
					try {
						...
						if (
							tx['asset-transfer-transaction']?.['asset-id'] ===
								Number(selectedToken.algorandAssetId) &&
			...
			<div class="flex flex-col md:flex-row md:items-end md:justify-between space-y-2 md:space-y-0">
				<div class="flex items-center space-x-2">
					...
					<div class="relative" bind:this={tokenSelectorRef}>
						...
						{#if showTokenSelector}
							<div
								class="absolute z-50 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 transform transition-all duration-200 origin-top"
							>
								<ul class="py-1">
				...
				<p class="text-xs md:text-sm text-gray-500 dark:text-gray-400">
					Powered by <a
						...
						target="_blank"
						class="text-blue-500 hover:underline">Aramid Bridge</a
					>
				</p>
			</div>
		</div>
			...
			{#if bridgeStatus === 'idle'}
				<div class="space-y-6">
						...
						<div class="relative w-full md:w-1/2 group">
								...
								<div class="flex flex-col items-center space-y-2 md:space-y-3">
									...
									<div class="text-sm text-gray-600 dark:text-gray-400 flex items-center">
										...
										<span class="font-medium text-blue-600 dark:text-blue-400"
											>{selectedToken.bridgedSymbol}</span
										>
							...
							{#if !canSignTransactions}
								...
								>
									...
									<div
										class="absolute bottom-full left-1/2 transform -translate-x-1/2 border-8 border-transparent border-b-gray-900 dark:border-b-gray-700"
									></div>
								</div>
							{/if}
						</div>
					...
					{#if !isVoiToAlgorand}
						<div class="space-y-2">
							<div class="flex items-center space-x-2">
								...
								>
									Source <span class="text-blue-600 dark:text-blue-400 font-semibold">Algorand</span
									> Address
								</label>
							...
							<div class="relative">
															...
															{#if 'owner' in result && (result.properties?.verified?.avatar || result.properties?.userDefined?.avatar)}
																<img
																	...
																	on:error={(e) => {
																		...
																		(target.nextElementSibling as HTMLDivElement).style.display =
																			'flex';
																	}}
																/>
															{:else}
								...
								{#if algorandSourceResolvedName}
									<div class="mt-2 flex items-center space-x-2">
										<div
											class="w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center overflow-hidden"
										>
								...
								{#if isValidSourceAddress}
									<div class="mt-2">
										...
										{:else if algorandSourceBalance !== null}
											...
											>
												Available balance: {algorandSourceBalance.toFixed(6)}
					...
					<div class="space-y-2">
						...
						>
							Destination <span
								class={`${isVoiToAlgorand ? 'text-blue-600 dark:text-blue-400' : 'text-emerald-600 dark:text-emerald-400'} font-semibold`}
							>
							...
							<div class="relative">
								...
								{#if isVoiToAlgorand && isValidDestination}
									<div class="mb-4">
										{#if isCheckingOptIn}
											<p class="text-sm text-gray-500 dark:text-gray-400">
												Checking if address is opted into {selectedToken.bridgedSymbol}...
											</p>
										{:else if !isOptedIn}
											<div
											...
											>
												<p class="text-sm text-yellow-700 dark:text-yellow-200 mb-2">
													This address has not opted into {selectedToken.bridgedSymbol}. The
													recipient must opt in to asset {selectedToken.algorandAssetId} before receiving
													tokens.
												</p>
										...
										{:else}
											<p class="text-sm text-green-600 dark:text-green-400">
												✓ Address is opted into {selectedToken.bridgedSymbol}
											</p>
										{/if}
									</div>
								{/if}
										...
										<ul class="max-h-48 overflow-auto py-1">
											{#each uniqueConnectedWallets as address, i}
												<li>
													...
													>
														<div
															class="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center bg-emerald-100 dark:bg-emerald-900"
														>
															<svg
					...
					<div class="space-y-2">
						...
						{#if isVoiToAlgorand}
							<div class="mb-2">
								...
								{:else if voiBalance !== null}
									<p class="text-sm text-gray-500 dark:text-gray-400">
										Available balance: {voiBalance.toFixed(6)}
										{isVoiToAlgorand ? selectedToken.symbol : selectedToken.bridgedSymbol}
									</p>
								{/if}
							</div>
						...
						<div class="space-y-1">
							<p class="text-sm text-gray-500 dark:text-gray-400">
								Bridge fee: {displayFeeAmount.toFixed(6)}
								{isVoiToAlgorand ? selectedToken.symbol : selectedToken.bridgedSymbol} (0.1%)
							</p>
							<p class="text-sm text-gray-500 dark:text-gray-400">
								You will receive: {displayDestinationAmount.toFixed(6)}
								{isVoiToAlgorand ? selectedToken.bridgedSymbol : selectedToken.symbol}
							</p>
						</div>
					</div>
					{#if !isVoiToAlgorand}
						<div class="text-center">
							<p class="text-sm text-gray-500 dark:text-gray-400">
								You will scan the generated QR code or open the link with your Algorand wallet
							</p>
						</div>
					{/if}
					...
					<Modal bind:open={showQrCode} size="sm" autoclose={false}>
						<div class="p-3 md:p-4">
							<div class="flex justify-between items-center mb-3 md:mb-4">
								<h3 class="text-lg md:text-xl font-medium text-gray-900 dark:text-white">
									Scan QR Code
								</h3>
							</div>
					...
					<Modal
						bind:open={showOptInQrCode}
						size="sm"
						autoclose={false}
						on:close={handleOptInQrCodeClose}
					>
						<div class="p-4">
				...
				<div class="space-y-6">
					...
					<div class="text-center">
						{#if bridgeStatus === 'signing'}
							...
							{#if isVoiToAlgorand}
								<p class="text-sm text-gray-500 dark:text-gray-400">
									Please review and sign the transaction in your <span
										class="text-emerald-600 dark:text-emerald-400 font-medium">VOI Network</span
									> wallet
								</p>
							{:else}
								<p class="text-sm text-gray-500 dark:text-gray-400">
									Please scan the QR code with your <span
										class="text-blue-600 dark:text-blue-400 font-medium">Algorand</span
									> wallet (Pera or Defly) to create and sign the transaction
								</p>
								<p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
									You can also click "Open in Pera or Defly Wallet" if you're on mobile
								</p>
							{/if}
						...
						{:else if bridgeStatus === 'error'}
							...
							<p class="text-sm text-red-500 dark:text-red-400">
								{bridgeError || 'An error occurred during the bridge process'}
							</p>
					...
					{#if bridgeStatus === 'bridging' || bridgeStatus === 'completed'}
						<BridgeAnimation
							...
							destinationToken={isVoiToAlgorand
								? selectedToken.bridgedSymbol
						...
						<div class="space-y-4 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
							<div>
								<label
									for="sourceTxId"
									class="block text-sm font-medium text-gray-700 dark:text-gray-300"
									>Source Transaction ID (on {isVoiToAlgorand ? 'Voi' : 'Algorand'})</label
								>
								<div class="mt-1 flex items-center space-x-2">
									<code
										id="sourceTxId"
										class="text-sm bg-gray-100 dark:bg-gray-900 px-2 py-1 rounded"
										>{bridgeDetails.sourceTxId}</code
									>
									<a
										href={isVoiToAlgorand
											? `https://explorer.voi.network/explorer/transaction/${bridgeDetails.sourceTxId}`
											: `https://allo.info/tx/${bridgeDetails.sourceTxId}`}
										target="_blank"
							...
							{#if bridgeDetails.destinationTxId}
								<div>
									<label
										for="destinationTxId"
										class="block text-sm font-medium text-gray-700 dark:text-gray-300"
										>Destination Transaction ID (on {!isVoiToAlgorand ? 'Voi' : 'Algorand'})</label
									>
									<div class="mt-1 flex items-center space-x-2">
										<code
											id="destinationTxId"
											class="text-sm bg-gray-100 dark:bg-gray-900 px-2 py-1 rounded"
											>{bridgeDetails.destinationTxId}</code
										>
										<a
											href={!isVoiToAlgorand
												? `https://explorer.voi.network/explorer/transaction/${bridgeDetails.destinationTxId}`
												: `https://allo.info/tx/${bridgeDetails.destinationTxId}`}
											target="_blank"
											rel="noopener noreferrer"
											class="text-blue-500 hover:text-blue-600"
										>
