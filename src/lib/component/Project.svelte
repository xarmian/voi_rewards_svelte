<script lang="ts">
	...
	let steps = [
		...
		},
		{
			...
			additionalInfo2:
				'It may take a few hours for telemetry to be detected after starting your node.'
		},
		{
			...
			if (wallet) {
				...
				if (points && points.length > 0) {
					...
					if (points[points.length - 1].health >= 5.0) {
						...
						steps[2].additionalInfo2 =
							'Maintain health score above 5.0 to earn points at end of epoch (Sunday 23:59 UTC).';
						currentWeekHealthy = true;
					} else {
						if (points[points.length - 1].hours < 168) {
							steps[2].additionalInfo2 =
								'Health score is an average and may take up to a week to reach its full value.';
						} else {
							steps[2].additionalInfo2 =
								'Health score must be above 5.0 to earn points at end of epoch (Sunday 23:59 UTC).';
						}
					}
				}
			}
	...
	async function getNautilusPoints() {
		try {
			...
			for (let i = 0; i < project.quests.length; i++) {
				...
				if (
					data.results.find(
						(result: { key: string; value: string }) => result.key === quest.name + ':' + wallet
					)
				) {
					quest.earned = 1;
				} else {
	...
	async function getKibisisPoints() {
		try {
			...
			for (let i = 0; i < project.quests.length; i++) {
				...
				if (
					data.quests.find((result: { id: string; total: number; completed: boolean }) => {
	...
	async function getHighforgePoints() {
		try {
			...
			for (let i = 0; i < project.quests.length; i++) {
				...
				if (
					data.find((result: { quest: string; points: number; completed?: boolean }) => {
	...
	function calculateChubsSharePoints(transferData: TransferData): WalletPoints[] {
		...
		// Convert to array of objects
		const result: WalletPoints[] = Array.from(walletPoints, ([wallet, points]) => ({
			wallet,
			points
		}));
		return result;
...
<div class="dark:text-white p-8 sm:rounded-lg my-4">
	<h2 class="text-3xl font-bold">
		...
		{:else if project.url}
			<a
				class="text-blue-900 hover:text-blue-700 underline cursor-pointer"
				target="_blank"
				href={project.url}
			>
				{project.title}
		...
		<div class="mb-4 md:mb-0 w-2/5 self-start">
			...
			{#if project.title == 'Voi Network'}
				<div class="text-red-800">
					NOTE: Nodes that join the network after July 31st will not be eligible for TestNet Phase 2
					reward points.
				</div>
			{/if}
			{#if !project.realtime}
				<div class="text-xs text-red-800">
					Live quest tracking is not yet available for this project.
				</div>
			{/if}
		</div>
...
<div
	class="rounded-lg bg-white dark:bg-opacity-70 text-[#41137E] font-extrabold text-lg flex flex-col sm:flex-row justify-center mx-8 p-4 space-x-4 items-center"
>
...
<div class="flex flex-col justify-center text-[#41137E]">
	<div class="flex flex-wrap justify-around p-4">
		{#each localProject.quests as quest, i}
			<div
				class="{quest.name != 'run_a_node'
					? 'w-full sm:w-1/2 md:w-1/3'
					: ''} bg-[#F2EAFF] dark:bg-opacity-80 rounded-lg overflow-hidden m-2 p-8 flex flex-col justify-between"
			>
				<div class="">
				...
				{:else}
					<div class="m-8 w-1/2 place-self-center">
						{#if quest.guide}
							<a
								class="flex justify-between rounded-lg text-white px-4 py-3 bg-[#6F2AE2] text-xl place-items-center"
								target="_blank"
								href={quest.guide}
							>
								<div>View Guide</div>
						...
						{#if !project.realtime || quest.earned == -1 || !quest.name}
							<div class="text-xs text-red-500 flex flex-row">
								...
								<InfoButton noAbsolute={true}>
									<p class="text-xs">
										We are currently unable to track the completion status for this quest. Quests or
										actions listed should still be tracked if done properly, and may be visible on
										other platforms such as Galxe, on the Project's own page, or not available yet.
										A link to the project's Galaxe page is available above this quest table.
									</p>
								</InfoButton>
							</div>
						{:else if !wallet}
					...
					<div class="p-4">
						NOTICE: If multiple wallets are staked to the same node, the points will be divided
						among the wallets.
					</div>
				{/if}
			</div>
