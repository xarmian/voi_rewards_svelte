...
>
	<div
		class="p-4 h-full bg-gray-100 dark:bg-slate-900 flex flex-col justify-between space-y-2 cursor-pointer"
	>
		...
		{#if project.title == 'Voi Network'}
			<div class="text-xs text-red-800">
				NOTE: New nodes started after July 31st do not earn TestNet Phase 2 points.
			</div>
		{/if}
		<div>
			...
			{:else if project.realtime ?? false}
				<p
					class="text-center text-xs py-2 rounded bg-green-200 dark:bg-green-900 text-green-700 dark:text-green-200"
				>
					Realtime Data ⚡
				</p>
			{/if}
