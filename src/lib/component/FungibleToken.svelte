<script context="module">
	...
	function calculateUserTokenShare(token: LPToken, tokenType: 'A' | 'B'): number {
		try {
			...
			const tokenBalance =
				tokenType === 'A' ? Number(token.poolInfo.tokABalance) : Number(token.poolInfo.tokBBalance);

			// Get the token decimals
			const tokenDecimals =
				tokenType === 'A' ? token.poolInfo.tokADecimals : token.poolInfo.tokBDecimals;
...
<script lang="ts">
			...
			if (!creator || !totalSupply) {
				if (typeof token === 'object' && 'type' in token && token.type === 'arc200') {
					...
					const response = await fetch(
						`https://voi-mainnet-mimirapi.voirewards.com/arc200/tokens?contractId=${token.id}`
					);
	...
	$: poolShare =
		isLPToken(token) && token.poolInfo
...
<div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-2 md:p-4 relative">
	...
	<div class="flex items-start space-x-4 flex-col">
		<div class="flex flex-row items-center gap-4 justify-start">
			{#if isLPToken(token) && token.poolInfo}
				{@const tokAId =
					token.poolInfo.tokBId === '390001' ? token.poolInfo.tokBId : token.poolInfo.tokAId}
				{@const tokBId =
					token.poolInfo.tokBId === '390001' ? token.poolInfo.tokAId : token.poolInfo.tokBId}
				<div class="relative w-16 h-16">
					<div class="absolute top-0 left-0 w-12 h-12 rounded-lg backdrop-blur-sm">
						<img
							src={tokenImages.get(tokAId) ||
								(tokAId === '390001'
									? '/icons/voi-token.png'
									: `https://asset-verification.nautilus.sh/icons/${tokAId === '302190' ? '395614' : tokAId}.png`)}
					...
					<div class="absolute bottom-0 right-0 w-12 h-12 rounded-lg backdrop-blur-sm">
						<img
							src={tokenImages.get(tokBId) ||
								(tokBId === '390001'
									? '/icons/voi-token.png'
									: `https://asset-verification.nautilus.sh/icons/${tokBId === '302190' ? '395614' : tokBId}.png`)}
					...
					<div class="w-full h-full rounded-lg backdrop-blur-sm">
						<img
							src={tokenImages.get(token.id) ||
								(token.id === '390001'
									? '/icons/voi-token.png'
									: `https://asset-verification.nautilus.sh/icons/${token.id === '302190' ? '395614' : token.id}.png`)}
							alt={token.name}
							class="w-full h-full rounded-lg object-cover"
			...
			<div class="flex-1">
				<div class="flex items-center justify-between">
					...
					{#if isLPToken(token) && apr !== null && apr !== 0}
						<span
							class="text-sm px-2 py-1 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 rounded-full"
						>
				...
				<div class="mt-2 space-y-2 text-sm">
					...
					{:else}
						<p
							...
							title={token.balance / Math.pow(10, token.decimals) + ' ' + token.symbol}
						>
							Balance: {formatNumber(token.balance / Math.pow(10, token.decimals))}
							{token.symbol}
						</p>
		...
		{#if isExpanded}
			{#if !isLPToken(token)}
				<div
					class="mt-3 space-y-2 border-t border-gray-200 dark:border-gray-600 pt-3 flex flex-col"
				>
							...
							<p class="text-gray-700 dark:text-gray-200">
								{#if totalSupply}
									<span class="group relative inline-block">
										{formatLargeNumber(totalSupply / Math.pow(10, token.decimals))}
										{token.symbol}
									</span>
								{:else}
									...
					...
					{#if token.approvals && token.approvals.filter((approval) => approval.amount !== '0').length > 0}
						<div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
							<p class="text-gray-500 dark:text-gray-400 mb-2">
								...
								Incoming Approvals ({token.approvals.filter((approval) => approval.amount !== '0')
									.length})
							</p>
									...
									<div class="bg-gray-100 dark:bg-gray-600 p-2 rounded-lg text-xs">
										...
										<div class="flex justify-between">
											...
											<span class="text-gray-800 dark:text-gray-200">
												{formatNumber(Number(approval.amount) / Math.pow(10, token.decimals))}
												{token.symbol}
											</span>
										</div>
										<div class="flex justify-between">
										...
										<div class="flex justify-between items-center mt-2">
											...
											{#if canSignTransactions}
												<button
													...
													class="px-2 py-1 text-xs font-medium text-white {claimSuccess &&
													claimingApproval === approval.transactionId
														? 'bg-green-600'
														: 'bg-blue-600 hover:bg-blue-700'} rounded transition-colors flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
												...
												>
													{#if claimingApproval === approval.transactionId}
														...
														{:else}
															<div
																class="w-3 h-3 border-t-2 border-r-2 border-white rounded-full animate-spin mr-1"
															></div>
															Claiming...
														{/if}
													{:else}
						...
						<div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
							<div class="flex justify-between items-center mb-2">
								<p class="text-gray-500 dark:text-gray-400">
									...
									Outgoing Approvals {token.outgoingApprovals &&
									token.outgoingApprovals.filter((approval) => approval.amount !== '0').length > 0
										? `(${token.outgoingApprovals.filter((approval) => approval.amount !== '0').length})`
										: ''}
								</p>
								<button
								...
								{#each token.outgoingApprovals.filter((approval) => approval.amount !== '0') as approval}
									<div class="bg-blue-50 dark:bg-blue-900/30 p-2 rounded-lg text-xs">
										...
										<div class="flex justify-between">
											<span class="text-gray-600 dark:text-gray-300">Amount:</span>
											<span class="text-gray-800 dark:text-gray-200">
												{(Number(approval.amount) / Math.pow(10, token.decimals)).toLocaleString()}
												{token.symbol}
											</span>
										</div>
									</div>
									...
									{formatNumber(
										(poolShare || 0) *
											(token.poolInfo.provider === 'humble'
												...
												: Number(token.poolInfo.tokABalance) /
													Math.pow(10, token.poolInfo.tokADecimals))
									)}
								</p>
							</div>
							<div>
									...
									{formatNumber(
										(poolShare || 0) *
											(token.poolInfo.provider === 'humble'
												...
												: Number(token.poolInfo.tokBBalance) /
													Math.pow(10, token.poolInfo.tokBDecimals))
									)}
								</p>
							</div>
							<div class="col-span-2">
		...
		<div class="flex items-center gap-3 text-xs font-mono pt-2 place-self-center">
			<a
				href={`https://voiager.xyz/token/${token.id}`}
				target="_blank"
			...
			{#if isLPToken(token)}
				...
				<a
					href={token.poolInfo.provider === 'humble'
						? `https://voi.humble.sh/#/pool/add?poolId=${token.poolId}`
						: `https://voi.nomadex.app/liquidity/${token.poolId}/add`}
					target="_blank"
				...
				<a
					href={token.poolInfo.provider === 'humble'
						? `https://voi.humble.sh/#/pool/remove?poolId=${token.poolId}`
						: `https://voi.nomadex.app/liquidity/${token.poolId}/remove`}
					target="_blank"
					class="flex flex-col items-center group"
					rel="noopener noreferrer"
				>
			...
			{:else if token.poolId}
				<span class="text-gray-400">|</span>
				<a
					href={`https://voi.humble.sh/#/swap?poolId=${token.poolId}`}
					target="_blank"
					class="flex flex-col items-center group"
					rel="noopener noreferrer"
				>
			...
			{#if !isLPToken(token)}
				<span class="text-gray-400">|</span>
				<button
					class="flex flex-col items-center group {canSignTransactions
						? ''
			...
			{/if}
			<span class="text-gray-400">|</span>
			<button class="flex flex-col items-center group" on:click={() => (showTransfersModal = true)}>
	...
	{#if isLPToken(token) && token.poolInfo.provider}
		<div class="absolute bottom-3 left-3">
			...
			{:else if token.poolInfo.provider === 'nomadex'}
				<a
					href={`https://voi.nomadex.app/${token.poolInfo.tokAType}/${token.poolInfo.tokAId}/${token.poolInfo.tokBType}/${token.poolInfo.tokBId}`}
					target="_blank"
					rel="noopener noreferrer"
				>
					<div class="w-6 h-6 rounded backdrop-blur-sm">
