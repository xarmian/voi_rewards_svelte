<script lang="ts">
	...
	$: filteredNftTokens = allNftTokens.filter((nft) => {
		...
		const matchesCollection =
			selectedCollection === 'all' || nft.collection?.name === selectedCollection;
	...
	async function fetchNFTs() {
		...
		try {
			...
			const uniqueContractIds = [
				...new Set(data.tokens.map((token: any) => token.contractId.toString()))
			] as string[];
			...
			allNftTokens = data.tokens.map((token: any) => {
				try {
					...
					const collectionName =
						finalMetadata.name?.replace(/(\d+|#)(?=\s*\S*$)/g, '') ??
			...
			const envoiTokens = allNftTokens.filter(
				(token) => token.contractId === '797609' || token.contractId === '876578'
			);
	...
	<div class="flex flex-col sm:flex-row gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
		<div class="flex-1">
			<p class="text-sm text-gray-600 dark:text-gray-300 mb-2">
				Looking for NFTs? Check out these marketplaces and resources:
			</p>
				...
				<div class="flex items-center space-x-4">
					...
					<div class="flex-1 min-w-0">
						...
						<p class="text-sm text-gray-500 dark:text-gray-400">
							Token ID: {nft.tokenId.length > 6 ? nft.tokenId.slice(0, 6) + '...' : nft.tokenId}
						</p>
						...
						<div class="mt-2 space-y-1 text-sm">
							{#if nft.isListed}
								<p class="text-green-600 dark:text-green-400">
									Listed: ${nft.listingPrice?.toLocaleString() ?? 0}
								</p>
							{/if}
