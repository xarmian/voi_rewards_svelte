...
export function convertVoiQuotedCandlesToUsd(candles: OHLCVData[], voiUsd: VoiUsdPoint[]): OHLCVData[] {
...
export function clampCandlesByMAD(candles: OHLCVData[], k = 6): OHLCVData[] {
  ...
  return candles.map((cd, idx) => ({ ...cd, open: oc[idx], high: hc[idx], low: lc[idx], close: cc[idx] }));
}
