...
export function getBasePair(pair: string): string {
	...
	const baseToken = Object.values(TOKENS).find((token) => token.equivalents.includes(base));
	const quoteToken = Object.values(TOKENS).find((token) => token.equivalents.includes(quote));
