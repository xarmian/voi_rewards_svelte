...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	...
	if (!isValid) {
		return new Response(
			JSON.stringify({
				error: 'Invalid wallet token. Please re-authenticate your Voi Account and try again.'
			}),
			{
				status: 401
			}
	...
	const { error: walletError } = await supabasePrivateClient.from('addresses').upsert(
		{
			user_id: user.id,
			address: walletId,
			chain: 'VOI',
