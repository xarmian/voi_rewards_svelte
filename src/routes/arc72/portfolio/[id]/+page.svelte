<script lang="ts">
	...
	let formattedWallet =
		walletId.length > 8 ? `${walletId.slice(0, 8)}...${walletId.slice(-8)}` : walletId;
</script>
...
<Breadcrumb aria-label="Navigation breadcrumb" solid>
	...
	<BreadcrumbItem href="/arc72/portfolio/{walletId}" class="hover:text-blue-800">
		...
		</svelte:fragment>Portfolio (<A
			href="https://explorer.voi.network/explorer/account/{walletId}"
			target="_blank">{walletNFD ?? formattedWallet}</A
		>)
	</BreadcrumbItem>
</Breadcrumb>
<div class="text-center">
	...
	<Tabs
		style="underline"
		defaultClass="flex rounded-lg divide-x rtl:divide-x-reverse divide-gray-200 shadow dark:divide-gray-700 justify-center"
	>
		<TabItem open>
			<div slot="title">
				<div class="inline">Portfolio</div>
				<Indicator color="blue" size="xl" class="text-xs font-bold text-white"
					>{tokens.length}</Indicator
				>
			</div>
			<div class="m-4">
				{#each tokens as token}
					<div class="m-4">
						<TokenComponent {token}></TokenComponent>
					</div>
				{/each}
				{#if isLoaded && tokens.length == 0}
					<div class="text-2xl font-bold">
						No tokens found! Want to get some? <A href="https://shellyssandbox.xyz/" target="_blank"
							>Check out the ARC-72 Marketplace</A
						>
					</div>
				{/if}
			</div>
