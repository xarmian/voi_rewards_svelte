<script lang="ts">
	...
	const goToMarketplace = () => {
		if (token)
			window.open(
				`https://shellyssandbox.xyz/#/nft/collection/${token.contractId}/token/${token.tokenId}`,
				'_blank'
			);
	};
	...
	const goToContract = () => {
		if (token)
			window.open(
				`https://explorer.voi.network/explorer/application/${token.contractId}/transactions`,
				'_blank'
			);
	};
...
<div class="m-5">
	<div class="button-bar">
		{#if isMobile}
			<div class="hamburger-container">
				...
				{#if isMenuOpen}
					<div class="menu">
						<button
							class="mb-1 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500"
							on:click={goToMarketplace}>Marketplace</button
						>
						<button
							class="mb-1 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500"
							on:click={goToContract}>Contract</button
						>
						<button
							class="bg-gray-200 dark:bg-gray-600 opacity-50 !cursor-not-allowed"
							on:click={goToProjectPage}>Project Page</button
						>
					</div>
				{/if}
			</div>
		{:else}
			<button
				class="mr-2 mb-1 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500"
				on:click={goToCollection}><i class="fas fa-arrow-left"></i> Collection</button
			>
			<button
				class="mr-2 mb-1 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500"
				on:click={goToMarketplace}>Marketplace</button
