<script lang="ts">
	...
	$: if (selectedWallet && snapshot) {
		totalBalance =
			(snapshot.find((s) => s.account === selectedWallet)?.voiBalance ?? 0) +
			(snapshot.find((s) => s.account === selectedWallet)?.viaBalance ?? 0);
...
<div class="flex flex-col bg-white dark:bg-gray-800">
	<div class="flex flex-col mt-4">
		...
		<p class="place-self-center">
			The Voi Testnet Phase 1 Snapshot occurred on May 1, 2024 at 00:00:00 UTC at block # {snapshot_block}
		</p>
		<br />
		<div
			class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4 w-3/4 place-self-center"
			role="alert"
		>
			<p>
				This is an unofficial resource and is not affiliated with Voi Network or the Voi Foundation.
				The data and calculator below attempt to be as accurate as possible based on available
				information, however it is not guaranteed to be accurate. Please verify with official
				sources before making any decisions.
			</p>
			<br />
			<p>
				As always please visit the <a
					href="https://discord.gg/vnFbrJrHeW"
					target="_blank"
					class="underline text-yellow-700">Voi Discord</a
				>
				for more specific help, and find the
				<a
					href="https://medium.com/@voifoundation/testnet-phases-announcement-338b929798bb"
					target="_blank"
					class="underline text-yellow-700">official Phase 1 announcement here</a
				>.
			</p>
		</div>
	</div>
	...
	<div class="dashboard justify-evenly flex flex-row">
		...
		<Card class="bg-gray-100 dark:bg-gray-700 h-42 w-64 m-2 relative">
			<InfoButton>
				This is the total number of MainNet tokens that will be distributed to eligible wallets
				based on the snapshot for TestNet Phase #1.
			</InfoButton>
			<div class="cardInner">
				<div class="cardInner">
					<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
	...
	<br />
	<div
		class="inline-flex flex-col p-8 rounded-2xl bg-slate-100 dark:bg-slate-700 place-self-center place-items-center"
	>
		<div class="relative w-full">
			{#if selectedWallet}
				<button
					class="absolute top-0 right-0 border-gray-400 dark:border-gray-300 border hover:bg-gray-300 text-gray-400 dark:text-white w-8 h-8 rounded-full flex items-center justify-center"
					on:click={() => {
						selectedWallet = null;
						totalBalance = null;
					}}
				...
				<h1
					class="text-2xl font-bold underline tracking-tight text-gray-900 dark:text-white flex flex-col place-items-center"
				>
					<div>
						{selectedWallet.substring(0, 8)}...{selectedWallet.substring(
							selectedWallet.length - 8,
				...
				{#if !snapshot.find((s) => s.account == selectedWallet)}
					<div>
						<h1
							class="text-xl font-bold tracking-tight text-gray-900 dark:text-white flex place-content-center"
						>
							Wallet Not Found in Eligible Snapshot List. Input amount below to calculate Mainnet
							Allocation.
						</h1>
					</div>
				{/if}
			...
			{#if selectedWallet && snapshot.find((s) => s.account == selectedWallet)}
				...
				<Card class="bg-green-100 dark:bg-green-700 h-42 w-60 m-2 relative">
					<div class="cardInner">
						...
						<p class="font-normal text-gray-700 dark:text-gray-400 leading-tight text-lg">
							{(
								snapshot.find((s) => s.account === selectedWallet)?.voiBalance +
								snapshot.find((s) => s.account === selectedWallet)?.viaBalance
							).toLocaleString()} VOI/VIA
						</p>
					</div>
					<InfoButton
						>This is the total combined VOI and VIA tokens in the wallet when the snapshot was
						taken. For the purposes of allocating tokens on Mainnet, 1 VOI = 1 VIA.</InfoButton
					>
				</Card>
				<i class="fas fa-arrow-right"></i>
				<Card class="bg-blue-100 dark:bg-blue-700 h-42 w-60 m-2 relative">
					<InfoButton
						>The Mainnet Airdrop is calculated as a percentage of the total eligible VOI and VIA
						distributed in Phase 1, multiplied by the total Phase 1 allocation of 150 Million.</InfoButton
					>
					<div class="cardInner">
						<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
		...
		<div class="flex flex-row">
			<Card class="bg-blue-100 dark:bg-blue-700 h-42 w-60 m-2 relative">
				<InfoButton>
					When tokens are locked, the full future bonus amount is allocated and made available for
					staking on a participation node, to earn node rewards. This number reflects the total
					amount with the bonus applied, which may be used to earn node participation rewards.
				</InfoButton>
				<div class="cardInner">
					<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
						Stakable Balance
			...
			<Card class="bg-blue-100 dark:bg-blue-700 h-42 w-60 m-2 relative">
				<InfoButton>
					After the selected lock-up period, tokens will vest at a rate of 1/12 per month for one
					year. This number reflects the amount a user may withdraw each month after the lock-up
					period.
				</InfoButton>
				<div class="cardInner">
					<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
		...
		<div class="flex flex-col">
			...
			<ul class="max-w-xl">
				...
				<li>
					Use the slider below to simulate potential block rewards based on total Online Stake and
					potential block reward emission rate per year
				</li>
				<li class="text-yellow-500 dark:text-yellow-300">
			...
			<div class="flex flex-row space-x-2">
				...
				<div class="flex flex-col self-center space-y-1">
					<div class="w-96">
						<div class="flex flex-row items-center justify-between">
							<div class="flex flex-row items-center">
								...
								<InfoButton noAbsolute={true}>
									This is the total amount of tokens staked on the network. The higher the total
									stake, the lower the individual rewards.
								</InfoButton>
							</div>
							<i class="fas fa-arrow-right"></i>
							<div class="flex flex-row items-center">
						...
						<RangeSlider
							bind:values={totalStake}
							min={100}
							max={2000}
							formatter={(v) => (v < 1000 ? v + 'M' : v / 1000 + 'B')}
							step={100}
							float={false}
							first="label"
							last="label"
					...
					<div class="w-96 self-center">
						<div class="flex flex-row items-center justify-between">
							<div class="flex flex-row items-center">
								...
								<InfoButton noAbsolute={true}>
									This is the total amount of tokens that will be distributed as block rewards over
									the course of a year. The higher the emission rate, the higher the rewards.
								</InfoButton>
							</div>
							<i class="fas fa-arrow-right"></i>
							<div class="flex flex-row items-center">
