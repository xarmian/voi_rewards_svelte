<script lang="ts">
	...
	$: {
		...
		for (let i = 0; i < $sortItems.length; i++) {
			...
			item.epoch_block_rewards =
				Math.floor(
					Math.floor((totalBlockRewards / totalBlocks) * item.block_count * Math.pow(10, 7)) / 10
				) / Math.pow(10, 6);
			item.block_rewards = item.block_count * rewardPerBlock;

			item.total_rewards =
				Math.round((item.block_rewards + item.health_rewards) * Math.pow(10, 6)) / Math.pow(10, 6);
		...
		filterItems = $sortItems.filter(
			(item: {
				proposer: string;
				nfd: string | undefined;
				nodes: { node_name: string | null }[];
			}) => {
				const matchesSearch =
	...
	columns.push({
		id: 'block_count',
		desc: 'Blocks',
		tooltip: 'Total blocks produced by each wallet so far this Epoch'
	});
	columns.push({
		id: 'block_rewards',
		desc: 'Est. Earned Reward',
		tooltip: 'Estimated block rewards earned by each wallet so far this Epoch'
	});
</script>
...
<div class="overflow-auto ml-0 mr-0 md:ml-6 md:mr-6 flex flex-col">
	<div class="flex justify-between items-center gap-4">
		<div class="flex flex-col md:flex-row items-center gap-4 flex-1">
			<TableSearch
				placeholder="Filter by Address or enVoi name"
				hoverable={true}
				bind:inputValue={searchTerm}
				innerDivClass="w-full"
				inputClass="w-64 sm:w-80 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 ps-10 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
			/>
				...
				>
					<span class="flex flex-col md:flex-row items-center">
						{#if column.id === 'proposer'}
							...
							<button class="md:ml-4" on:click|stopPropagation aria-label="Show ENS">
						...
						{#if column.tooltip}
							...
							<Tooltip
								defaultClass="py-2 px-3 text-sm font-medium w-64"
								triggeredBy="#tooltip_{i}"
								type="auto">{column.tooltip}</Tooltip
							>
						{/if}
					</span>
					{#if column.id === 'block_rewards' || column.id === 'health_points' || column.id === 'total_rewards'}
						<button
							...
							on:click|stopPropagation={() => downloadCSV(column.id)}
		...
		{:else}
			<TableBody tableBodyClass="divide-y">
					...
					<TableBodyRow on:click={() => toggleRow(i)}>
						<TableBodyCell tdClass="px-2 py-2 whitespace-nowrap font-medium">
							...
							{#if item.expires_in_hrs <= 0}
								<i
									class="fas fa-ban text-red-500 ml-2"
									title="Consensus Participation Key has Expired"
								></i>
							{:else if item.key_expiring7d}
								<i
									class="fas fa-exclamation-triangle text-yellow-400 ml-2"
									title="Consensus Participation Key is Expiring Soon"
								></i>
							{/if}
						</TableBodyCell>
						<TableBodyCell tdClass="px-2 py-2 whitespace-nowrap font-medium" title={item.proposer}>
								...
								>
									...
									{:else if showWalletNFD && item.nfd !== undefined}
										<span class="inline-block"
											>{item.nfd.length > 16 ? item.nfd.substring(0, 10) + '...' : item.nfd}</span
										>
									{:else}
										{item.proposer.substring(0, 4)}...{item.proposer.substring(
											item.proposer.length - 4
										)}
									{/if}
							...
							{#if item.showNicknameInput}
								<button class="mt-2" on:click|stopPropagation aria-label="Set Nickname">
									<NicknameManager
						...
						<TableBodyCell tdClass="px-2 py-2 whitespace-nowrap font-medium"
							>{item.block_count}</TableBodyCell
						>
						<TableBodyCell tdClass="px-2 py-2 whitespace-nowrap font-medium"
							>{item.block_rewards.toFixed(2)} VOI</TableBodyCell
						>
					</TableBodyRow>
					{#if expandedRow === i}
						<TableBodyRow>
								...
								<div class="px-2 py-3 m-4" transition:slide={{ duration: 300, axis: 'y' }}>
									<div class="space-y-3">
										...
										<div>
											...
											<button
												on:click|stopPropagation={() => goto(`/wallet/${item.proposer}`)}
												class="text-blue-500 hover:text-blue-600 underline">{item.proposer}</button
											>
										...
										<div>
											<Label defaultClass="text-sm font-medium inline-block w-40"
												>Account Balance:</Label
											>
											<span class="text-gray-600 dark:text-gray-400"
												>{selectedBalance.toLocaleString()} VOI</span
											>
										</div>

										<div>
											<Label defaultClass="text-sm font-medium inline-block w-40"
												>Est. Reward (full epoch):</Label
											>
											<span class="text-gray-600 dark:text-gray-400"
												>{item.epoch_block_rewards.toLocaleString()} VOI</span
											>
										</div>
										...
										<div>
											...
											<span class="text-gray-600 dark:text-gray-400"
												>{((item.epoch_block_rewards / Number(selectedBalance)) * 52 * 100).toFixed(
													2
												)}%</span
											>
										</div>
				...
				<TableBodyRow class="bg-gray-50 dark:bg-gray-900">
					...
					<TableBodyCell colspan={2} class="p-2">Blocks:</TableBodyCell>
					<TableBodyCell class="p-2">
						{filterItems.reduce((sum, item) => sum + item.block_count, 0)}
					</TableBodyCell>
					<TableBodyCell colspan={1} class="p-2">
						{Math.round(
							filterItems.reduce((sum, item) => sum + item.block_rewards, 0)
						).toLocaleString()} VOI
					</TableBodyCell>
				</TableBodyRow>
			</TableBody>
	...
	<div class="flex justify-between items-center gap-4">
		...
		<button
			class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed"
			on:click={() =>
				(currentPage = Math.min(Math.ceil(filterItems.length / itemsPerPage), currentPage + 1))}
...
<Modal
	bind:open={viewWallet}
	autoclose
	size="lg"
	outsideclose
	class="bg-gray-100 dark:bg-black rounded-xl shadow-lg transition-all duration-300 w-full max-w-4xl mx-auto border-2 border-gray-300"
>
