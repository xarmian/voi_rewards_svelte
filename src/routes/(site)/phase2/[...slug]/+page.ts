...
export const load = async ({ params }) => {
	...
	const project =
		projects.find((p) => p.column === parts[1]) ?? projects.find((p) => p.id === Number(parts[1]));
	...
	if (project) {
		...
		pageMetaTags.imageUrl = project.logo
			? 'https://voirewards.com' + project.logo.replace(/\.(svg|webp)$/, '.png')
			: 'https://voirewards.com/logos/Voi_Logo_White_on_Purple_Background.png';
	}
