<script lang="ts">
	...
	const updateEligibility = (wallet: string) => {
		fetch(`/api/eligibility?wallet=${wallet}`)
			.then((r) => r.json())
			.then((d) => {
...
<div class="flex flex-col bg-[rgb(111,42,226)]">
	<div class="flex flex-col">
		<img
			src={VoiLogoStatic}
			alt="Voi Logo"
			class="-my-32 h-[30rem] z-0 self-center object-cover object-center"
			style="clip-path: inset(180px 0 180px 0)"
		/>
	...
	<div class="hidden flex space-x-4 place-self-center mb-8">
		<a
			href="https://voiager.org/get-started"
			target="_blank"
			class="flex items-center justify-center px-4 py-2 bg-blue-500 text-white font-medium rounded-md shadow hover:bg-blue-700 transition duration-150 ease-in-out"
		>
			<i class="fas fa-faucet mr-2"></i> $VOI Faucet
		</a>
		<a
			href="https://faucet.nautilus.sh{selectedWallet ? '?account=' + selectedWallet : ''}"
			target="_blank"
			class="flex items-center justify-center px-4 py-2 bg-green-500 text-white font-medium rounded-md shadow hover:bg-green-700 transition duration-150 ease-in-out"
		>
	...
	<div class="flex flex-col w-full bg-white dark:bg-gray-800 place-items-center">
		<div class="px-2 m-4 w-full sm:w-1/2">
			<div class="text-center mb-4">
				Phase 2 has completed and Mainnet is live! Search for your address below to view your
				estimated Phase 2 rewards.
			</div>
			<WalletSearch
				onSubmit={(v) => {
					goto(`/phase2/${v}`);
			...
			<div class="flex flex-col place-items-center pt-4">
				{#if data.props.wallet}
					<div class="items-center sm:items-start sm:ml-4 m-1 mb-8">
						{#if !dataUpdating}
							<button
								class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-4 px-8 rounded-lg text-3xl"
								on:click={() => (showAllocationModal = true)}
							>
								🎉 View your Phase 2 Estimated Rewards! 🎉
							</button>
						{:else}
							<div
								class="hidden flex-col place-items-center text-xl bg-yellow-100 dark:bg-yellow-900 p-6 rounded-lg shadow-md"
							>
								<i
									class="fas fa-sync-alt animate-spin text-3xl mb-4 text-yellow-600 dark:text-yellow-400"
								></i>
				...
				{#if data.props.leaderboardData}
					<div class="flex flex-wrap place-self-center">
						<div class="flex flex-col items-center sm:items-start sm:ml-4 m-1">
							<div
								class="p-4 rounded-lg bg-gray-50 dark:bg-gray-900 shadow-md h-48 w-48 flex flex-col items-center justify-center"
							>
								<div class="text-2xl font-semibold text-gray-800 dark:text-white">
									{data.props.leaderboardData.network ?? 0}
								</div>
								<a
									class="text-blue-500 underline hover:text-blue-400 pointer"
									href="/wallet/{selectedWallet}">Node Points</a
								>
							</div>
						</div>
						<div class="flex flex-col items-center sm:items-start sm:ml-4 m-1">
							...
							>
								<div class="flex space-x-1">
									<div class="text-2xl font-semibold text-gray-800 dark:text-white">
										{(data.props.leaderboardData.total ?? 0) -
											(data.props.leaderboardData.network ?? 0)}
									</div>
						...
						<div class="flex flex-col items-center sm:items-start sm:ml-4 m-1">
							...
							>
								<div class="text-2xl font-semibold text-gray-800 dark:text-white">
									{data.props.leaderboardData.row_number ?? 0}
								</div>
								<!--<div class="text-2xl font-semibold text-gray-800 dark:text-white">Not Available</div>-->
								<a class="text-blue-500 underline hover:text-blue-400 pointer" href="/leaderboard"
									>Leaderboard Position</a
								>
							</div>
						</div>
						<div class="flex flex-col items-center sm:items-start sm:ml-4 m-1">
							...
							>
								{#if loaded}
									{#if !isEligible}
										<div
											class="relative text-2xl font-semibold text-yellow-800 dark:text-yellow-300 text-center"
										>
											Address NOT Registered
										</div>
										<div class="text-sm">For Phase 2</div>
										<a class="text-blue-500 underline hover:text-blue-400 pointer" href="/accounts"
											>Manage Account</a
										>
										<div class="absolute top-1 right-1">
											<InfoButton noAbsolute={true}>
												<div class="text-sm">
													Register your addresses at the link above to be eligible for the Phase 2
													Testnet Rewards.
												</div>
											</InfoButton>
										</div>
									{:else}
										<div
											class="text-xl font-semibold text-green-800 dark:text-green-300 text-center"
										>
											🎉 Address Registered 🎉
										</div>
										<div class="text-sm">For Phase 2</div>
									...
									{/if}
								{:else}
									<div class="text-xl font-semibold text-gray-800 dark:text-white text-center">
										Checking Phase 2 Registration Status...
									</div>
								{/if}
							</div>
						</div>
					...
					<div class="items-center sm:items-start sm:ml-4 m-1">
						<div class="text-sm">
							...
							<Time
								timestamp={data.props.leaderboardData.last_modified}
...
{#if showAllocationModal}
	<div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-20">
		<div
			class="bg-white dark:bg-black p-8 rounded-lg shadow-lg max-h-[90vh] overflow-y-auto relative"
		>
			<div class="flex flex-col h-full max-h-[80vh]">
				<div class="flex justify-between items-center mb-4">
					...
					<button
						...
						on:click={() => (showAllocationModal = false)}>X</button
					>
				</div>
				{#if !data.props.questData}
					<div class="flex flex-col place-items-center">
						<div class="text-xl font-bold max-w-96">
							Token Allocation not found for this Address. If this address is linked to a Discord
							account, your combined token allocation will be listed under your Primary address.
							<br />
				...
				{:else}
					<div class="overflow-y-auto flex-grow pr-4">
						...
						>
							...
							<p class="mb-2">
								The token numbers displayed on the Testnet Phase 2 airdrop site are estimates due
								to:
							</p>
							<ul class="list-disc list-inside mb-2">
								...
								<li>
									Data analysis for proof of humanity levels, where each earns a different bonus
								</li>
							</ul>
							<p>
								The score multiplier per proof of humanity level isn't final until October 28th when
						...
						<div class="mb-4 relative">
							<div class="flex items-center absolute top-0 right-0 text-sm">
								...
								<InfoButton noAbsolute={true}>
									<div class="text-sm">
										Data is refreshed periodically. If your Discord account was connected or Human
										Verification was completed recently, the status and estimated rewards may not be
										updated until the next refresh.
									</div>
								</InfoButton>
							</div>
							...
							<div class="flex flex-col space-y-2 ml-4">
								<div class="flex items-center">
									...
									{:else}
										<span class="text-red-500 flex flex-row"
											>✗ Not Linked
											<InfoButton noAbsolute={true}>
												<div class="text-sm">
													Receive a reward boost by linking your Discord Account by going to <a
														href="https://voirewards.com/accounts"
														target="_blank">https://voirewards.com/accounts</a
								...
								<div class="flex items-center">
									...
									{:else}
										<span class="text-red-500 flex flex-row"
											...
											<InfoButton noAbsolute={true}>
												<div class="text-sm">
													Receive a reward boost by being assigned the Phase 2 Discord Role. To get
													the role, join the Voi Discord and go to the #phase2 channel for
													instructions on performing human verification.
												</div>
											</InfoButton>
										</span>
									{/if}
								...
								<div class="text-center text-red-800 dark:text-red-400">
									...
									<Time
										timestamp={data.props.questData?.last_modified}
										format="MMM D, YYYY h:mm A"
										relative
									/>
								</div>
						...
						<table class="w-full">
							...
							<tbody>
								{#if false}
										...
										{#if data.props.questList.filter((quest) => quest.project === project.id).length > 0}
											...
											{#each data.props.questList.filter((quest) => quest.project === project.id) as quest}
												<tr>
													...
													<td class="text-right"
														>{(data.props.questData?.quest_data?.[quest.id] ?? 0) *
															(data.props.questList.find((q) => q.id === quest.id)?.reward ?? 0) ??
															0}</td
													>
												</tr>
											{/each}
									...
									<tr>
										<td colspan="3" class="text-base font-bold pt-0 pb-4"
											>Proof of Humanity Multiplier</td
										>
									...
									<tr>
										...
										<td class="text-right pt-0 pb-4">
											{(
												data.props.totalPoints *
												data.props.discordMultiplier *
												data.props.humanMultiplier
											).toFixed(6)}
										</td>
									</tr>
								{/if}
								<tr>
									<td colspan="3" class="text-xl font-bold pt-2 pb-4"
										>Estimated $VOI Phase 2 Reward</td
									>
						...
						<div class="flex flex-col place-items-center mt-4">
							<div class="flex flex-col place-items-center">
								<h1
									class="text-4xl font-bold tracking-tight text-gray-900 dark:text-white flex place-content-center"
								>
									Lockup Option
								</h1>
								<ul class="max-w-xl">
									<li>
										Airdrop may be locked for one to five years for a 10%-20% componded bonus per
										year
									</li>
							...
							<div class="w-80">
								<RangeSlider
									...
									pips={true}
									all="label"
									on:change={(e) => (lockYears = e.detail.value)}
								/>
							</div>
							<div class="flex flex-row">
								<Card class="bg-blue-100 dark:bg-blue-700 h-42 w-60 m-2 relative">
									<InfoButton>
										...
										available for staking on a participation node, to earn node rewards. This number
										reflects the total amount with the bonus applied, which may be used to earn node
								...
								<Card class="bg-blue-100 dark:bg-blue-700 h-42 w-60 m-2 relative">
									<InfoButton>
										After the selected lock-up period, tokens will vest at a rate of 1/12 per month
										for one year. This number reflects the amount a user may withdraw each month
				...
				<div class="mt-4">
					<button
						...
						on:click={() => (showAllocationModal = false)}>Close</button
					>
				</div>
			</div>
		</div>
