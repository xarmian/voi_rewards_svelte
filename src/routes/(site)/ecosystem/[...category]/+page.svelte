<script lang="ts">
	...
	$: projectsByCategory = categorySort.reduce(
		(acc, category) => {
			let filteredProjects = projects.filter(
				(project) => project.category === category && project.status === 'active'
			);
									...
									>
										{#if project.logo}
											<img
												src={project.logo}
												alt={project.title}
												class="mr-2 object-contain h-[{project.title === 'Lute' ? '4rem' : '5rem'}]"
											/>
										{:else}
											<div
												class="w-8 h-8 mr-2 bg-purple-600 rounded-full flex items-center justify-center text-white font-bold"
											>
												{project.title[0]}
											</div>
										{/if}
	...
	<section class="py-20 bg-purple-100">
		<div class="container mx-auto px-4 text-center">
			...
			<p class="text-xl text-gray-700 mb-8">
				Explore the full range of projects and opportunities in the Voi ecosystem.
			</p>
			<a
				href="/directory"
				rel="noopener noreferrer"
