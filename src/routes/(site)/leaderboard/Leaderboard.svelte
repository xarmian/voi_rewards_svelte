	...
	<thead class="table-header text-white bg-[#2c037a] dark:bg-[#672ed9]">
		<tr>
			...
			<th class="px-1 sm:px-6 py-3 text-left text-xs font-medium uppercase tracking-wider"
				>User Account</th
			>
			<th class="px-1 sm:px-6 py-3 text-left text-xs font-medium uppercase tracking-wider hidden"
				>$POINTS</th
			>
			<th class="px-1 sm:px-6 py-3 text-left text-xs font-medium uppercase tracking-wider"
				># Actions</th
	...
	<tbody class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
		{#each data as row, i}
			<tr class={i % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-slate-800'}>
				<td
					class="px-1 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white"
				>
					{row.row_number ?? i + 1}
				</td>
				<td
					class="px-1 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300"
					title={row.address}
				>
				...
				<td
					class="px-1 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 hidden"
				>
					{row.points_tokens ?? 0}
				</td>
