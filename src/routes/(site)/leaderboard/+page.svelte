	...
	$: {
		if (searchTerm.length > 0) {
			supabasePublicClient
				...
				.order('total_quest_points', { ascending: false })
				.limit(50)
				.then(({ data, error }) => {
					if (error) {
						console.error(error);
			...
			<div class="flex flex-col sm:flex-row sm:justify-between">
				<div class="w-full">
					<div class="text-red-800 dark:text-red-500 font-bold text-2xl text-center mb-4">
						Testnet Phase 2 has ended. Voi is now in Mainnet. This leaderboard may update as Phase 2
						data is processed.
					</div>
					...
					<div class="mt-4 mb-1 flex justify-between w-full">
						...
						{#if ranks}
							<div class="place-self-end">
								Last updated: <Time
									timestamp={ranks[0].last_modified}
