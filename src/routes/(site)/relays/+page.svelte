<script lang="ts">
	...
	let dates: string[] = [];
	let searchTerm = '';
	let sortField: 'host_name' | 'total_peers' | 'percentage' | 'reward' | `daily_${string}` =
		'host_name';
	...
	function updateFilteredData() {
		filteredData = peerData.filter(
			(host) => searchTerm === '' || host.host_name.toLowerCase().includes(searchTerm.toLowerCase())
		);
...
<div class="bg-blue-50 dark:bg-blue-900/80 min-h-screen">
	<div class="container mx-auto px-2 sm:px-4 py-4 sm:py-8 text-gray-900 dark:text-white">
			...
			<div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-6 sm:mb-8">
				...
				>
					...
					<p class="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">
						{weekStats.rewardPool.toLocaleString()} VOI
					</p>
				</div>
				<div
					class="bg-white dark:bg-gray-800 rounded-lg p-3 sm:p-4 shadow-sm border border-gray-200 dark:border-gray-700"
				>
					...
					<p class="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">
						{weekStats.totalHosts}
					</p>
				</div>
				<div
					class="bg-white dark:bg-gray-800 rounded-lg p-3 sm:p-4 shadow-sm border border-gray-200 dark:border-gray-700"
				>
					<h3 class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-1">
						Avg Peers per Relay
					</h3>
					<p class="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">
						{formatNumber(weekStats.avgPeersPerHost)}
					</p>
				</div>
				<div
					class="bg-white dark:bg-gray-800 rounded-lg p-3 sm:p-4 shadow-sm border border-gray-200 dark:border-gray-700"
				>
					<h3 class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-1">
						Avg Reward per Relay
					</h3>
					<p class="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">
						{formatNumber(weekStats.avgRewardPerHost)} VOI
					</p>
				</div>
			</div>
		{/if}
		...
		{:else if filteredData.length}
			<div class="overflow-x-auto -mx-2 sm:mx-0">
				<div class="inline-block min-w-full align-middle">
					<table
						class="min-w-full bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-sm border border-gray-200 dark:border-gray-700"
					>
						...
						<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
							{#each filteredData as host}
								<tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
									<td
										class="sticky left-0 bg-white dark:bg-gray-800 px-3 sm:px-4 py-2 font-mono text-gray-900 dark:text-gray-200 text-xs sm:text-sm whitespace-nowrap"
										>{host.host_name}</td
									>
									{#each dates as date}
										<td
											class="px-3 sm:px-4 py-2 text-center text-gray-700 dark:text-gray-300 text-xs sm:text-sm"
										>
											{host.daily_peers[date] ? formatNumber(host.daily_peers[date]) : '-'}
										</td>
									{/each}
									<td
										class="px-3 sm:px-4 py-2 text-center font-medium text-gray-900 dark:text-gray-200 text-xs sm:text-sm"
									>
									...
									<td
										class="px-3 sm:px-4 py-2 text-center text-gray-900 dark:text-gray-200 text-xs sm:text-sm"
									>
							...
							<tr class="bg-gray-50 dark:bg-gray-700/50 font-medium">
								<td
									class="sticky left-0 bg-gray-50 dark:bg-gray-700/50 px-3 sm:px-4 py-2 font-mono text-gray-900 dark:text-gray-200 text-xs sm:text-sm whitespace-nowrap"
									>Total</td
								>
								{#each dates as date}
									<td
									...
									>
										{Math.round(
											filteredData.reduce((sum, host) => sum + host.daily_peers[date], 0) || 0
										)}
									</td>
								{/each}
								<td
		...
		{:else}
			<p class="text-center text-gray-500 dark:text-gray-400 text-sm sm:text-base">
