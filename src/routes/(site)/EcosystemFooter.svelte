	...
	<div class="relative">
		<div class="flex overflow-x-auto space-x-2 py-4 px-2 scrollbar-hide">
			{#each projects as project}
				...
				>
					<div
						class="w-24 h-24 rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-700 flex items-center justify-center border border-gray-200 dark:border-gray-700 hover:border-purple-500 dark:hover:border-purple-500 transition-colors"
					>
						{#if project.logo}
							<div class="w-full h-full flex items-center justify-center">
								<div
									class="p-4 rounded-lg {project.logo_dark_mode === 'light-bg'
										? 'dark:bg-white/20'
								...
								>
									<img
										src={project.logo}
										alt={project.title}
										class="max-w-full max-h-full object-contain {project.logo_dark_mode === 'invert'
					...
					<p
						class="text-xs text-center mt-2 text-gray-600 dark:text-gray-400 font-medium group-hover:text-purple-500 break-words hyphens-auto min-h-10 transition-colors duration-300"
					>
						{project.title}
					</p>
				</button>
