<script lang="ts">
	...
	$: if (projects.length > 0) {
		...
		const keptProjects = projects
			.filter((project) => keep.includes(project.title))
			.sort((a, b) => keep.indexOf(a.title) - keep.indexOf(b.title));
...
<div
	class={`${selectedProjectId ? 'blur-sm' : ''} flex flex-col items-center`}
	style="background-color: rgb(111,42,226)"
>
	<div class="flex flex-col h-1/2">
		<img
			src={VoiLogoStatic}
			alt="Voi Logo"
			class="-my-36 h-[40rem] z-0 self-center object-cover object-center"
	...
	<div
		class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-8 w-full max-w-4xl"
		role="alert"
	>
		<p class="font-bold">Important Notice:</p>
		<p>
			Testnet Phase 2 has concluded. The quests listed below are no longer active for reward
			purposes.
		</p>
		<p class="mt-2">
			To view your estimated token rewards for Testnet Phase 2 participation, please visit the <a
				href="/phase2"
				class="underline font-semibold hover:text-red-800">Phase 2 Rewards page</a
			>.
		</p>
	</div>
	...
	<div class="restofpage">
		<div class="hero-section py-16">
			...
			<div class="text-2xl my-6 w-full sm:w-1/2">
				Get started by downloading a wallet, securing Testnet tokens, and familiarizing yourself
				with the Voi ecosystem.
			</div>
			<div class="cta-buttons flex-row flex-wrap justify-center mt-10 mb-10 max-w-[60rem]">
				...
				<a href="https://voiager.org/get-started" target="_blank" class="cta-button"
					>$VOI<br />Faucet</a
				>
				<a href="https://faucet.nautilus.sh/" target="_blank" class="cta-button">$VIA<br />Faucet</a
				>
				<a
					href="https://airtable.com/apphFYuejZFJJG0i6/shru2v6BXxUaAEU7O"
					target="_blank"
					class="cta-button">Ecosystem Directory</a
				>
				<a
					href="https://voinetwork.github.io/voi-swarm/getting-started/introduction/"
					target="_blank"
					class="cta-button">Node Running</a
				>
				<a href="https://x.com/Voi_Net" target="_blank" class="cta-button"
					>Voi Twitter/<i class="fa-brands fa-x-twitter"></i></a
				>
				<a href="https://discord.gg/vnFbrJrHeW" target="_blank" class="cta-button"
					>Voi Discord <i class="fa-brands fa-discord"></i></a
				>
			</div>
		</div>

		<div
			class="p-4 sm:p-12 flex flex-col justify-center items-center text-[#6F2AE2] bg-white dark:bg-black"
		>
			<div class="flex flex-col justify-center items-center sm:w-3/4 mb-14">
				...
				<p class="text-lg text-gray-400">
					Quests help test the network and introduce you to the budding Voi ecosystem. They are fun,
					educational, and a great way to learn about Voi while earning rewards. Points earned from
					quests will correlate to an airdrop following Voi's move to Mainnet, anticipated for later
					this year.
				</p>
			</div>
			...
			<div class="flex flex-col justify-center items-center sm:w-3/4">
				<button
					class="py-4 px-8 text-xl bg-blue-500 text-white rounded-lg"
					on:click={() => goto('/leaderboard')}>View Leaderboard</button
				>
			</div>
			...
			<div class="flex flex-wrap place-content-center pt-8">
				{#each filteredProjects as project, i}
					<div
						class="project flex p-2 flex-col bg-gray-100 dark:bg-gray-800 sm:w-[20rem] rounded-lg w-[343px] h-[540px] m-1 sm:m-4"
					>
						<div class="place-self-end text-[#41137E] px-3 rounded-lg bg-[#65DBAB] w-fit">
							{project.type}
						</div>
						<div class="p-4 h-full flex flex-col justify-between">
							<div>
								{#if project.title == 'Social Quests'}
									<div class="flex flex-col h-20 justify-center">
										<svg xmlns="http://www.w3.org/2000/svg" height="20px" class="block dark:hidden"
											><path
												fill="#000"
												fill-rule="evenodd"
								...
								{#if project.title === 'Voi Network'}
									<p class="text-sm text-red-800 dark:text-red-400 mt-2">
										NOTE: Nodes that join the network after July 31st will not be eligible for
										TestNet Phase 2 reward points.
									</p>
								{/if}
							</div>
							<div class="flex flex-col space-y-2 h-48">
								...
								{:else}
									<button
										on:click|stopPropagation={() => (selectedProjectId = project.id)}
										class="flex h-14 w-full bg-white text-[#41137E] dark:bg-gray-700 dark:text-[#D4BFF6] rounded-md font-bold px-4 py-2 items-center space-x-2 border border-gray-200 dark:border-gray-700 cursor-pointer hover:border-gray-300 dark:hover:border-gray-500"
	...
	<div
		class="flex p-4 sm:p-20 text-white flex-col justify-center items-center w-full {filteredProjects.length %
			2 ===
		0
			? 'bg-[#672ed9]'
			: 'bg-[#2C037A]'}"
	>
		...
		<p class="text-xl mb-8">
			Check out our Phase 2 Quest Tracker for more Projects, Quests, and Real-time Status Tracking.
		</p>
		<a
			href="/phase2"
			class="bg-[#65DBAB] text-[#672ed9] font-bold py-2 px-4 rounded-full shadow-md hover:bg-[#672ed9] hover:text-white w-1/2 text-center"
			>Go to Phase 2 Quest Tracker</a
		>
	</div>
</div>
{#if selectedProjectId}
