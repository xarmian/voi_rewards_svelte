	...
	function filterGroupedFAQ(groupedFAQ: any, category: string, query: string) {
		...
		Object.entries(groupedFAQ).forEach(([groupCategory, items]) => {
			if (category === 'All' || category === groupCategory) {
				const filteredItems = (items as FAQItem[]).filter(
					(item: FAQItem) =>
...
<div
	class="bg-gradient-to-br from-purple-50 to-indigo-100 dark:from-gray-900 dark:to-purple-900 min-h-screen py-8 px-4 sm:px-6 lg:px-8"
>
	<div class="mx-auto flex flex-col lg:flex-row gap-8 place-items-start">
		<div class="lg:w-3/4">
			...
			<section class="bg-white dark:bg-gray-800 shadow-md rounded-xl overflow-hidden mb-8">
				<header
					class="bg-purple-600 dark:bg-purple-800 py-4 px-6 flex flex-wrap justify-between items-center gap-4"
				>
					...
					<div class="flex flex-wrap items-center gap-4">
						...
						<div class="relative">
							...
							/>
							<SearchOutline
								class="w-5 h-5 text-gray-500 dark:text-gray-400 absolute left-3 top-1/2 -translate-y-1/2"
							/>
						</div>
					</div>
				</header>
				...
				<div class="p-6">
					...
					{#if highlightedFAQ}
						<Accordion
							multiple
							class="divide-y divide-gray-200 dark:divide-gray-700 mb-8"
							id={slugify(highlightedFAQ.question)}
						>
								...
								<svelte:fragment slot="header">
									<div class="w-full flex items-center justify-between">
										<a
											...
											on:click={(e) =>
												handleFAQInteraction(e, highlightedFAQ?.question ?? '', 'link')}
										>
										...
										<div
											...
											tabindex="0"
											on:click={(e) =>
												handleFAQInteraction(e, highlightedFAQ?.question ?? '', 'copy')}
											on:keydown={(e) =>
												e.key === 'Enter' &&
												handleFAQInteraction(e, highlightedFAQ?.question ?? '', 'copy')}
								...
								{#each items as item}
									{#if item !== highlightedFAQ}
										<AccordionItem open={expandAll}>
											<svelte:fragment slot="header">
												<div class="w-full flex items-center justify-between">
													...
													<div
														...
														on:keydown={(e) =>
															e.key === 'Enter' && handleFAQInteraction(e, item.question, 'copy')}
														class="ml-2 p-2 text-purple-600 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-200 cursor-pointer"
														title="Copy link to clipboard"
													>
														<ClipboardOutline class="w-5 h-5" />
											...
											<div
												class="markdown whitespace-pre-wrap prose dark:prose-invert max-w-none py-4"
											>
							...
							{#each recentFAQs as faq}
								<li>
									<a
										...
										on:click={() => selectFaq(faq)}
										class="block hover:bg-gray-100 dark:hover:bg-gray-700 p-3 rounded-lg transition-colors"
									>
										<h3 class="text-lg font-semibold text-green-600 dark:text-green-400">
											<Markdown source={faq.question} renderers={{ link: ExternalLink }} />
										</h3>
			...
			<div class="bg-white dark:bg-gray-800 shadow-md rounded-xl overflow-hidden top-8">
				...
				<div class="p-4">
					...
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						class="block mt-4 text-center text-purple-600 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-200 font-semibold"
					>
						View all posts
					</a>
				</div>
...
<Drawer placement="right" width="w-80" bind:hidden={drawerHidden} id="sidebar-drawer">
	<div
		class="bg-gradient-to-br from-purple-50 to-indigo-100 dark:from-gray-900 dark:to-purple-900 h-full p-4 space-y-6"
	>
		<div class="bg-white dark:bg-gray-800 shadow-md rounded-xl overflow-hidden">
			<header class="bg-purple-600 dark:bg-purple-800 py-4 px-6">
				<h2 class="text-2xl font-bold text-white">Latest from Voi Foundation</h2>
...
{#if showToast}
	<Toast class="fixed bottom-5 right-5">
		<span class="font-semibold text-sm text-blue-500 dark:text-blue-400"
