...
interface HumblePool {
	...
	base_volume_1y: string;
	target_volume_1y: string;
...
[
  {
    "ticker_id": "395614_0",
    "base_currency": "aUSDC",
    "base_currency_id": "395614",
    "target_currency": "VOI",
    "target_currency_id": "0",
    "pool_id": "395553",
    "last_price": "1034.86065205898022189646",
    "base_volume": "122.52800224249084891404",
    "target_volume": "126799.40829614827077200865018092",
    "liquidity_in_usd": "25660.83502800000012707337968274",
    "update_datetime": "2025-07-09T13:48:30+0000",
    "high": "1050.1763451591657",
    "low": "1034.8606520589801",
    "base_volume_24h": "122.52800224249084891404",
    "target_volume_24h": "126799.40829614827077200865018092",
    "base_volume_7d": "2979.246311643040611886",
    "target_volume_7d": "3083104.78071122880742374346512878",
    "base_volume_30d": "25469.86307571265506777979",
    "target_volume_30d": "26357759.11038494176397168012194866",
    "base_volume_1y": "152524.74688231714554543776",
    "target_volume_1y": "157841859.01376563192575107289968426",
    "base_volume_all": "152524.74688231714554543776",
    "target_volume_all": "157841859.01376563192575107289968426",
    "base_price": "1034.86065205898022189646",
...
async function fetchHumbleData(): Promise<MarketSnapshot | null> {
	try {
		...
		const response = await fetch('https://mainnet-idx.nautilus.sh/integrations/coingecko/tickers');
		...
		// data is an array of pools
		const pool: HumblePool = data.filter((p: HumblePool) => p.pool_id === POOL_ID.toString())[0];
		...
		const price = 1 / parseFloat(pool.last_price);

		console.log('here4');
		// Get 24h volume in VOI and convert to USDC
		const volume24h = parseFloat(pool.base_volume_24h);

		// Get TVL in VOI and convert to USDC
		const tvl = parseFloat(pool.liquidity_in_usd);
...
async function fetchTinymanData(): Promise<MarketSnapshot | null> {
	try {
		...
		// Calculate price (USDC per VOI)
		const price =
			parseFloat(poolData.current_asset_2_reserves) / parseFloat(poolData.current_asset_1_reserves);
...
async function fetchPactFiData(): Promise<MarketSnapshot | null> {
	try {
		...
		for (const pool of pools) {
			...
			const voiAsset = pool.assets.find(
				(asset) => asset.unit_name === 'aVoi' || asset.name.includes('VOI')
			);
		...
		if (price === 0) {
			...
			const voiAsset = largestPool.assets.find(
				(asset) => asset.unit_name === 'aVoi' || asset.name.includes('VOI')
			);
...
serve(async (req: Request) => {
	...
	try {
		...
		const [humbleData, tinymanData, pactfiData, uniswapData, nomadexData] = await Promise.all([
			// fetchMEXCData(),
			fetchHumbleData(),
			fetchTinymanData(),
			fetchPactFiData(),
		...
		const snapshots = [humbleData, tinymanData, pactfiData, uniswapData, nomadexData].filter(
			(data): data is MarketSnapshot => data !== null
		);
