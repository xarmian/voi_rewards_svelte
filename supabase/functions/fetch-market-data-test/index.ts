...
async function fetchHumbleData(): Promise<MarketSnapshot[]> {
	try {
		...
		const voiUsdcPrice =
			parseFloat(voiUsdcPool?.poolBalA ?? '0') / parseFloat(voiUsdcPool?.poolBalB ?? '0');
...
async function fetchNomadexData(): Promise<MarketSnapshot | null> {
	try {
		...
		const tradingPair = await getOrCreateTradingPair(NOMADEX_EXCHANGE_ID, 'aUSDC', 'VOI', poolUrl);
...
async function fetchPactFiData(): Promise<MarketSnapshot | null> {
	try {
		...
		const price =
			(pool1.price * pool1VoiLiquidity + pool2.price * pool2VoiLiquidity) / totalVoiLiquidity;
